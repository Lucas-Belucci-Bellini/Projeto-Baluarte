...
| `RECOVERY-001` | Há drill local reproduzível sobre a ponte de backup, mas não há RPO/RTO, durabilidade ou recuperação remota aprovados | `npm run drill:v2:backup` passou; rollback de código existe; recuperação de dados de produção não foi demonstrada | Disaster recovery e declaração de produção ficam adiados | Gap operacional parcial/bloqueado |
