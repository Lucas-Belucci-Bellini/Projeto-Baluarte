...
**Base de trabalho:** `84804f124da9aff53b1c1f2327e319ed974782ac` (alterações de Color Studio ainda não publicadas)
**Status:** INVENTÁRIO ATUALIZADO — J1 do JARVIS, Modpack, Projetos, Zomboid, laboratório cripto completo, Calculadoras, utilitárias, Git Nexus Gate, Segurança, Banco, Centro Militar, Poder Militar, Comms, Baixar, Portas, Diagnóstico, Economia, Orçamentos Militares, Shadow, Triangulação, GeoPulse, JSON Studio, Batalha Naval, IA Proprietária, FFT e Color Studio foram implementados; **45 páginas canônicas** continuam no mapa abaixo.
...
| **Total** | **352** | **242** | **111** | **81** |

A soma de `src` e `v2` também pode ser lida de forma mais simples: existem **295 arquivos JS em `src`**, **56 em `v2`**, **109 wrappers de compatibilidade** e **111 implementações TypeScript canônicas**. As implementações já migradas cobrem o Core V1, Layout, **69 páginas**, adaptadores visuais/integrações, os contratos J1 e o Core V2 tipado; os wrappers permanecem para preservar os imports legados.

## 2. O que já não precisa ser convertido agora

Cento e nove arquivos JavaScript são wrappers de compatibilidade que reexportam uma implementação TypeScript. Eles continuam no repositório de propósito, porque páginas e testes legados ainda importam os caminhos `.js`.
...
Ainda existem **45 módulos de páginas em JavaScript canônico**. A migração deve ser feita por risco, não apenas por tamanho; Dossiê, Simbolos, Gerar Código, Git Helper, Dólar, Biblioteca, Academia, CiberSeg e Robotica já saíram desta contagem.
