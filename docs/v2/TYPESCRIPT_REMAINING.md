...
| **Total** | **352** | **209** | **144** | **115** |

A soma de `src` e `v2` também pode ser lida de forma mais simples: existem **295 arquivos JS em `src`**, **56 em `v2`**, **142 wrappers de compatibilidade** e **144 implementações TypeScript canônicas**. As implementações já migradas cobrem o Core V1, Layout, **102 páginas**, adaptadores visuais/integrações, os contratos J1, os painéis de mídia/IDE e o Core V2 tipado; os wrappers permanecem para preservar os imports legados.
...
| Próxima onda segura | `modelos-3d.js`, `mapa.js`, `radar.js` e contratos visuais | Ainda JS canônico | Médio a alto |
...
| Hubs e catálogos médios | `modelos-3d.js`, `mapa.js` e `radar.js` | Ainda JS canônico | Médio a alto |
