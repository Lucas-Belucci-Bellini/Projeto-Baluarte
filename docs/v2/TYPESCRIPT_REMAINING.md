...
| **Total** | **352** | **208** | **145** | **120** |

A soma de `src` e `v2` também pode ser lida de forma mais simples: existem **295 arquivos JS em `src`**, **56 em `v2`**, **143 wrappers de compatibilidade** e **145 implementações TypeScript canônicas**. As implementações já migradas cobrem o Core V1, Layout, **103 páginas**, adaptadores visuais/integrações, os contratos J1, os painéis de mídia/IDE e o Core V2 tipado; os wrappers permanecem para preservar os imports legados.

## 2. O que já não precisa ser convertido agora

Cento e quarenta e três arquivos JavaScript são wrappers de compatibilidade que reexportam uma implementação TypeScript. Eles continuam no repositório de propósito, porque páginas e testes legados ainda importam os caminhos `.js`.
...
| Próxima onda segura | `mapa.js`, `radar.js`, `radio.js` e contratos visuais | Ainda JS canônico | Médio a alto |
...
| Hubs e catálogos médios | `mapa.js`, `radar.js` e `radio.js` | Ainda JS canônico | Médio a alto |
