...
| **Total** | **352** | **207** | **146** | **121** |

A soma de `src` e `v2` também pode ser lida de forma mais simples: existem **295 arquivos JS em `src`**, **56 em `v2`**, **144 wrappers de compatibilidade** e **146 implementações TypeScript canônicas**. As implementações já migradas cobrem o Core V1, Layout, **104 páginas**, adaptadores visuais/integrações, os contratos J1, os painéis de mídia/IDE e o Core V2 tipado; os wrappers permanecem para preservar os imports legados.

## 2. O que já não precisa ser convertido agora

Cento e quarenta e quatro arquivos JavaScript são wrappers de compatibilidade que reexportam uma implementação TypeScript. Eles continuam no repositório de propósito, porque páginas e testes legados ainda importam os caminhos `.js`.
...
| Próxima onda segura | `radar.js`, `radio.js`, `musicas.js` e contratos de mídia | Ainda JS canônico | Médio a alto |
...
| Hubs e catálogos médios | `radar.js`, `radio.js` e `musicas.js` | Ainda JS canônico | Médio a alto |
