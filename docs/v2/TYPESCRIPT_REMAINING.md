...
**Base de trabalho:** `7e07cd8ee4c484d0fafb19884a11e79fe1b134e7` (alterações de Perfil ainda não publicadas)
**Status:** INVENTÁRIO ATUALIZADO — J1 do JARVIS, Modpack, Projetos, Zomboid, laboratório cripto completo, Calculadoras, utilitárias, Git Nexus Gate, Segurança, Banco, Centro Militar, Poder Militar, Comms, Baixar, Portas, Diagnóstico, Economia, Orçamentos Militares, Shadow, Triangulação, GeoPulse, JSON Studio, Batalha Naval, IA Proprietária, FFT, Color Studio, Morse standalone, Central de APIs e Perfil foram implementados; **42 páginas canônicas** continuam no mapa abaixo.
...
| **Total** | **352** | **239** | **114** | **86** |

A soma de `src` e `v2` também pode ser lida de forma mais simples: existem **295 arquivos JS em `src`**, **56 em `v2`**, **112 wrappers de compatibilidade** e **114 implementações TypeScript canônicas**. As implementações já migradas cobrem o Core V1, Layout, **72 páginas**, adaptadores visuais/integrações, os contratos J1 e o Core V2 tipado; os wrappers permanecem para preservar os imports legados.

## 2. O que já não precisa ser convertido agora

Cento e doze arquivos JavaScript são wrappers de compatibilidade que reexportam uma implementação TypeScript. Eles continuam no repositório de propósito, porque páginas e testes legados ainda importam os caminhos `.js`.
...
Ainda existem **42 módulos de páginas em JavaScript canônico**. A migração deve ser feita por risco, não apenas por tamanho; Dossiê, Simbolos, Gerar Código, Git Helper, Dólar, Biblioteca, Academia, CiberSeg e Robotica já saíram desta contagem.
