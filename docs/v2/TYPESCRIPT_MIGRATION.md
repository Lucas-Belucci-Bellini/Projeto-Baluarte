...
### Gates remotos do `main` após a onda militar estática

O código da onda foi publicado no commit [`22cd5c9a`](https://github.com/Lucas-Belucci-Bellini/Projeto-Baluarte/commit/22cd5c9afb8fcc38738a55c859a000ccd2d81f48). O `Core CI`, o `V2 Runtime`, o `CodeQL`, o `Arma 3 Data CI` e o `Vigia das rotas` passaram no mesmo SHA. O Vigia confirmou build, 98 rotas, navegação, caminho crítico, limpeza dos efeitos e sobrevivência à perda de rede. CI, V2 Core e V2 Validation permaneceram vermelhos pela mesma causa-raiz já catalogada: 61 diagnósticos em arquivos JavaScript/JSDoc da V2, sem referência às quatro páginas militares.
