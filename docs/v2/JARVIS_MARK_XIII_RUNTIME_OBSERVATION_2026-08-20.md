...
O harness V2 expõe `platformRuntimeObservation()` e o envelope `platformRuntimeObservationEnvelope()` somente para testes de integração. Ele observa o diagnóstico da Plataforma, classifica registros `healthy` do Registry como estado normal — não como incidentes adversos — e mantém `authority: not-authorized`. O envelope acrescenta redaction explícita, origem fixa `v2-harness`, nonce, digest SHA-256 e rejeição de retratos expirados ou alterados. O gate V2 confirma essa projeção junto com o boot, Runtime, Registry, router V1 e os demais contratos, totalizando `36/36` afirmações. Nenhum controle operacional foi delegado ao adaptador. Os testes unitários cobrem `healthy`, `degraded`, `failed`, `exhausted`, fallback, redaction, TTL, nonce, digest, tampering e replay.
