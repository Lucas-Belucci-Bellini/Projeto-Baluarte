...
When `--evidence <path>` is used, the Doctor validates the file size before parsing and accepts at most `256 KiB` and `100` records. Oversized input is rejected rather than truncated. This keeps externally supplied evidence bounded without treating a truncated report as valid.
