...
**SHA de referência:** `9b5b5d0555610f4468e83b3d2751d8421b779868`
**Último marco:** `claims-observation/v1` — projeção read-only de identidade e escopo publicada no main
**Data:** 2026-08-20
...
| PHASE 12 — JARVIS Foundation | **Parcial avançada — visual lightweight, server-health read-only, transporte redigido, integridade, claims read-only, severidade/fallback e adaptador V2 Mark XIII** | JARVIS, WebLLM, Hermes, voz, memória, Spotify, contratos, console integrado da rota `/jarvis`, benchmark antes/depois, 72/40 partículas, stride de conexões adaptativo, projeção `nucleo:status`/health manual, contrato `server-health/v1` em FastAPI/Vercel, envelope `platform-observation/v1` com TTL/redaction/SHA-256/anti-replay, `claims-observation/v1` com identidade e escopo limitado, severidade/fallback read-only e adaptador `projectPlatformDiagnostic()`; V7 standalone preservado | Auth/RLS real, assinatura/origem server-side, auditoria de consumidor, medição em hardware real e em paralelo com OpenClaw, Tool Registry, permission boundaries e fallback operacional autorizado |
