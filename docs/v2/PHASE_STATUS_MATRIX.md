...
| PHASE 12 — JARVIS Foundation | **Parcial avançada — visual, orçamento, observação, server-health read-only, severidade/fallback e adaptador V2 Mark XIII** | JARVIS, WebLLM, Hermes, voz, memória, Spotify, contratos, console integrado da rota `/jarvis`, benchmark, degradação visual por FPS/memória, projeção `nucleo:status`/health manual, contrato `server-health/v1` em FastAPI/Vercel, severidade/fallback read-only e adaptador `projectPlatformDiagnostic()`; V7 standalone preservado | Medir em hardware real, Tool Registry, permission boundaries, ponte autenticada para `PlatformDiagnostic`, claims e fallback operacional autorizado |
