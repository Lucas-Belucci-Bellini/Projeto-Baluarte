...
| PHASE 6 — Configuration | **Parcial** | Configuração e flags existentes no Core; redação de segredo fechada em `validacao()` e proveniência real no diagnóstico ([`CONFIG_SECRET_REDACTION_2026-08-24.md`](./CONFIG_SECRET_REDACTION_2026-08-24.md)), com 8 testes de contrato em `test/security/` e 6 de diagnóstico | Fonte única, versionamento e configuração de módulos; `v2/core/config.js` ainda não está ligado ao boot |
