...
| PHASE 12 — JARVIS Foundation | **Parcial avançada — visual, orçamento, observação, severidade/fallback e adaptador V2 Mark XIII** | JARVIS, WebLLM, Hermes, voz, memória, Spotify, contratos, console integrado da rota `/jarvis`, benchmark, degradação visual por FPS/memória, projeção `nucleo:status`/health manual, severidade/fallback read-only e adaptador `projectPlatformDiagnostic()`; V7 standalone preservado | Medir em hardware real, Tool Registry, permission boundaries, health server-side, claims e fallback operacional autorizado |
