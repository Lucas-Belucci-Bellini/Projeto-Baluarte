...
| PHASE 12 — JARVIS Foundation | **Parcial avançada — visual, orçamento adaptativo e observação Mark XIII** | JARVIS, WebLLM, Hermes, voz, memória, Spotify, contratos, console integrado da rota `/jarvis`, benchmark, degradação visual por FPS/memória e projeção `nucleo:status`/health manual; V7 standalone preservado | Medir em hardware real, Tool Registry, permission boundaries, health server-side, claims e fallback documentado |
