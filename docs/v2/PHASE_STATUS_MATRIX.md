...
**SHA de referência:** `76ca9e913ed3dc589c16762572930abc7e4fbe42`
**Último marco:** `server-claims/v1` — adaptador FastAPI/Vercel read-only publicado no main
**Data:** 2026-08-21
...
| PHASE 12 — JARVIS Foundation | **Parcial avançada — visual lightweight, server-health read-only, transporte redigido, integridade, claims read-only, adaptador server-side, severidade/fallback e adaptador V2 Mark XIII** | JARVIS, WebLLM, Hermes, voz, memória, Spotify, contratos, console integrado da rota `/jarvis`, benchmark antes/depois, 72/40 partículas, stride de conexões adaptativo, projeção `nucleo:status`/health manual, contrato `server-health/v1` em FastAPI/Vercel, envelope `platform-observation/v1` com TTL/redaction/SHA-256/anti-replay, `claims-observation/v1` com identidade e escopo limitado, adaptador `server-claims/v1` no FastAPI e Vercel, severidade/fallback read-only e adaptador `projectPlatformDiagnostic()`; V7 standalone preservado | Auth/RLS real, assinatura/origem server-side, auditoria de consumidor, CORS/rate limit, roles/expiração server-side, medição em hardware real e em paralelo com OpenClaw, Tool Registry, permission boundaries e fallback operacional autorizado |
