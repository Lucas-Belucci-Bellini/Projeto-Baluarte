...
**SHA de referência:** `f8614de2474258fd09ddd845da68c54b450226e4`
**Último marco:** `platform-observation/v1` — transporte redigido publicado no main
**Data:** 2026-08-20
...
O Projeto-Baluarte já possui uma fundação significativa: governança e documentação V2, Core inicial, Event Bus, Storage, permissões básicas, Runtime/Session/Bridge, integração V2, migração das páginas canônicas para TypeScript, fronteira Node/TypeScript dos geradores e automação diária. A baseline corrente passa `tipos:ts`, `tipos:v2`, testes 1110/1110, build, integração 32/32, smoke 99/99 e caminho crítico 15/15.
...
| PHASE 12 — JARVIS Foundation | **Parcial avançada — visual lightweight, server-health read-only, transporte redigido, severidade/fallback e adaptador V2 Mark XIII** | JARVIS, WebLLM, Hermes, voz, memória, Spotify, contratos, console integrado da rota `/jarvis`, benchmark antes/depois, 72/40 partículas, stride de conexões adaptativo, projeção `nucleo:status`/health manual, contrato `server-health/v1` em FastAPI/Vercel, envelope `platform-observation/v1` com TTL/redaction, severidade/fallback read-only e adaptador `projectPlatformDiagnostic()`; V7 standalone preservado | Auth/RLS, assinatura/origem/auditoria server-side, medição em hardware real e em paralelo com OpenClaw, Tool Registry, permission boundaries, claims e fallback operacional autorizado |
