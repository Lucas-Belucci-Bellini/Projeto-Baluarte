...
`LOCAL FAKE VALIDATED — NO REMOTE EXECUTION`
...
A implementação tipada está em `v2/data/staging-rls-matrix.ts`. O harness estrutural está em `test/security/staging-rls-matrix-contract.test.js`. O executor fake server-side local está em `v2/data/staging-rls-fake.ts` e seus contratos em `test/security/staging-rls-fake.test.js`. O fake valida as nove expectativas, evidencia estado antes/depois, mantém quatro identidades determinísticas e executa cleanup idempotente. Ele não cria usuários, não executa SQL remoto e não faz escrita em Supabase.
...
A matriz fake passou nos nove cenários declarados: leituras próprias foram permitidas, leituras cross-user ficaram vazias, updates/deletes cross-user foram negados, o operador conseguiu inserir post administrativo e o usuário comum foi negado. As fixtures `profiles`, `memories` e `mural_posts` são identificadas por `fixtureId`; a primeira limpeza remove as três linhas e a segunda não remove nada. A auditoria fake registra decisão, identidade lógica, tabela, operação, resultado e timestamp, sem token ou stack trace.

O fake é uma evidência de contrato, não uma prova de que as policies remotas estão verdes. Ele não interpreta SQL nem reproduz o mecanismo real do Postgres; por isso, qualquer divergência entre este executor e as migrations reais deve ser tratada como risco, não como aprovação.
...
Nenhuma conta foi criada, nenhum token foi solicitado, nenhuma migration foi aplicada remotamente, nenhum dado de produção foi consultado e nenhuma policy foi alterada. A matriz não declara que RLS remoto está verde; o fake apenas torna o contrato local explícito e auditável. A execução remota continua bloqueada até aprovação explícita do operador.
