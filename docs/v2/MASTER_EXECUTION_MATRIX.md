...
| 03 | Event Bus and Task Manager | `IN PROGRESS` — correlação e causação entregues; cancelamento já existia no escalonador | Core, lifecycle, observability | Eventos órfãos e retry inconsistente | Catálogo versionado; integração verde; `bus-correlacao` 13/13 com escopo restaurado, aninhamento e limite assíncrono cobrados | Payloads, correlation e causation no envelope; cadeia no log de erro | Latency budget não fechado | Event/runtime docs, [`EVENT_BUS_CORRELACAO_2026-08-24.md`](./EVENT_BUS_CORRELACAO_2026-08-24.md) | `da85a395` | Health e retry contract (política de repetição por classe de evento ainda não decidida) |
