...
Os marcos de promoção do projeto estão em [`docs/v2/RELEASE_PLAN.md`](docs/v2/RELEASE_PLAN.md). A versão web/desktop em preparação é `1.1.5`; a release pública `1.1.0` permanece no histórico, e `1.0.0` continua pulada como congelamento da arquitetura anterior. As notas da candidatura estão em [`docs/releases/v1.1.5.md`](docs/releases/v1.1.5.md). Nenhum marco é considerado publicado apenas por estar mergeable: o SHA e os checks precisam estar identificados.
...
O novo Master Super-Prompt é reconciliado pelo [`MASTER_EXECUTION_MATRIX.md`](docs/v2/MASTER_EXECUTION_MATRIX.md) e pelo [`MASTER_GAP_ANALYSIS.md`](docs/v2/MASTER_GAP_ANALYSIS.md). Esses documentos distinguem o que já existe no SHA corrente, o que está incompleto, o que é local-only, o que está bloqueado por dependências externas e o que pertence a versões futuras. A matriz não declara a V2 completa por quantidade de código: cada fase precisa de contrato, testes, segurança, documentação, publicação e verificação pós-merge.
