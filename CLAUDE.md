...
- **Ferramentas externas de IA** (GitNexus & cia): instaladas fora do git, em `.baluarte/tools/`. O repo versiona `config/ai-tools.json` + `scripts/sync-ai-tools.mjs`; playbook em **[`docs/local-ai-tools.md`](docs/local-ai-tools.md)** (`npm run tools:status` mostra o que está instalado). Repositório de terceiro **não** se copia pra dentro deste — foi assim que `GitNexus-1.6.7/` virou 139 MB no histórico.
