...
interface Theme {
  name: string;
  deep: number; high: number; hot: number; blade: number;
  rings: number; ringAlt: number; constel: number; dust: number;
  ember: number; bg: number;
  css: string; css2: string; dim: string; txt: string;
}

interface ViewSpec { name: string; dist: number; pitch: number; fov: number; auto: number }
interface RingSpec { r: number; count: number; len: number }
interface Bands { bass: number; mid: number; treble: number; level: number }
...
const CFG = {
  ...
  rings: [{ r: 3.15, count: 44, len: 0.34 },
          ...
          { r: 4.95, count: 68, len: 0.26 }] as RingSpec[],
...
class AudioEngine {
  ...
  mode: AudioMode = 'off';
  bands: Bands = { bass: 0, mid: 0, treble: 0, level: 0 };
  beat = false;
  fft = new Uint8Array(CFG.eqBars);
  label = '';
  ...
  private peak = { bass: 0.05, mid: 0.04, treble: 0.02, level: 0.04 };
  private mean = { bass: 0.02, mid: 0.02, treble: 0.01, level: 0.02 };
  private specRaw = new Float32Array(CFG.eqBars);
  private specPeak = 24;
  private beatCool = 0;
  private bpm = 84;
  private step = 0;
  ...
  private ensure(): AudioContext {
    ...
    bus.connect(master);
    this.ctx = ctx; this.master = master; this.analyser = an; this.musicBus = bus;
  ...
  /* ─ fontes ─ */
  toggleGenerative(): boolean {
    const ctx = this.ensure(); this.resume();
    if (this.mode === 'gen') { this.silence(); return false; }
    this.detach();
    this.mode = 'gen'; this.label = 'partitura generativa · baluarte';
    this.musicBus!.gain.setTargetAtTime(0.9, ctx.currentTime, 0.6);
    this.nextTime = ctx.currentTime + 0.08; this.step = 0;
    this.timer = window.setInterval(() => this.schedule(), 25);
    return true;
  }

  async playFile(file: File): Promise<string> {
    const ctx = this.ensure(); this.resume();
    this.detach();
    ...
    await el.play();
    this.mode = 'file'; this.label = file.name.replace(/\.[^.]+$/, '');
    return this.label;
  }
  ...
  async useMic(): Promise<boolean> {
    ...
    this.mode = 'mic'; this.label = 'entrada externa · microfone';
    return true;
  }

  private detach(): void {
    if (this.timer) { clearInterval(this.timer); this.timer = 0; }
    if (this.musicBus && this.ctx) this.musicBus.gain.setTargetAtTime(0, this.ctx.currentTime, 0.2);
  ...
  silence(): void { this.detach(); this.mode = 'off'; this.label = ''; }
  ...
  private emit(i: number, t: number): void {
    ...
    if (i % 16 === 0) this.pad(t, root, bar);
    if (i % 4 === 0) this.bassNote(t, root, i % 16 === 8 ? 1.5 : 1);
    if ([2, 3, 6, 10, 11, 14].indexOf(i % 16) >= 0) {
      const n = scale[(i * 5 + bar) % scale.length];
      this.pluck(t, root * 4 * Math.pow(2, n / 12));
    }
    if (i % 4 === 2) this.sizzle(t, 0.05);
    if (i % 16 === 15) this.sizzle(t, 0.16);
  }
  ...
  private bassNote(t: number, f: number, len: number): void {
    ...
    o.start(t); o.stop(t + 0.7 * len);
  }

  private pad(t: number, root: number, bar: number): void {
  ...
  private sizzle(t: number, amp: number): void {
  ...
  update(dt: number): void {
    ...
    if (!this.analyser || this.mode === 'off') {
      ...
      for (let i = 0; i < this.fft.length; i++) this.fft[i] = Math.max(0, this.fft[i] - 12);
      return;
    }
    const raw = this.raw!;
    ...
    let relBass = 0;
    const norm = (v: number, key: 'bass' | 'mid' | 'treble' | 'level', floor: number): number => {
    ...
    const ease = (cur: number, v: number): number =>
      cur + (v - cur) * (v > cur ? Math.min(1, dt * 22) : Math.min(1, dt * 5));
    ...
    if (relBass > 0.52 && eBass > this.peak.bass * 0.25 && this.beatCool <= 0) {
      this.beat = true; this.beatCool = 0.3;
    }
...
/* ═══ 3 · ESTADO ═══════════════════════════════════════════════════════════ */
let scene: Obj, camera: Obj, renderer: Obj, composer: Obj, bloomPass: Obj, compPass: Obj;
let coreGroup: Obj, crystal: Obj, plasma: Obj, heart: Obj, hitSphere: Obj, strip: Obj, scanDisc: Obj;
let eqMesh: Obj, fftTex: Obj;
const ringGroups: Obj[] = [], bladeMeshes: Obj[] = [], sparkMats: Obj[] = [], circles: Obj[] = [];
let constel: Obj, constelBase: Float32Array, constelVel: Float32Array;
let linkGeo: Obj, linkPairs: Int32Array, glyphs: Obj;
let arcGeo: Obj, arcMat: Obj, dust: Obj;
const waves: Obj[] = [];
...
let mouseX = -999, clock: Obj, ray: Obj, ndc: Obj, mw: Obj;
let SPRITE: Obj = null;

const hex = (v: number): string => '#' + v.toString(16).padStart(6, '0');
const rgba = (v: number, a: number): string =>
  'rgba(' + (v >> 16 & 255) + ',' + (v >> 8 & 255) + ',' + (v & 255) + ',' + a + ')';
function randDir(): Obj {
  const z = Math.random() * 2 - 1, a = Math.random() * Math.PI * 2, s = Math.sqrt(1 - z * z);
  return new THREE.Vector3(Math.cos(a) * s, Math.sin(a) * s, z);
}
function tint(mat: Obj, key: keyof Theme, uni?: string): Obj {
  themed.push({ mat, key, uni }); return mat;
}
function glowSprite(): Obj {
...
/* ═══ 4 · CONSTRUÇÃO ═══════════════════════════════════════════════════════ */
function init(): void {
  clock = new THREE.Clock(); ray = new THREE.Raycaster();
  ndc = new THREE.Vector2(); mw = new THREE.Vector3();
  ...
  buildCrystal(); buildPlasma(); buildEqualizer(); buildAstrolabe(); buildSparks();
  buildArcs(); buildStrip(); buildScanDisc(); buildWaves();
  buildConstellation(); buildGlyphs(); buildDust();
  postFX(); bind();
...
function buildCrystal(): void {
  ...
  for (let f = 0; f < tri; f++) {
    const s = Math.random();
    for (let v = 0; v < 3; v++) { bary[(f * 3 + v) * 3 + v] = 1; seed[f * 3 + v] = s; }
  }
  ...
  hitSphere = new THREE.Mesh(new THREE.SphereGeometry(2.3, 14, 10),
    new THREE.MeshBasicMaterial({ visible: false }));
  coreGroup.add(hitSphere);
}

/* plasma: pontos deslocados por ruído — grave dilata, médio agita */
function buildPlasma(): void {
  const geo = new THREE.BufferGeometry().setAttribute('position',
    new THREE.BufferAttribute(Float32Array.from(
      new THREE.IcosahedronGeometry(CFG.plasmaR, CFG.plasmaDetail).attributes.position.array), 3));
  const mat = new THREE.ShaderMaterial({
    uniforms: {
      uDeep: { value: new THREE.Color(TH.deep) }, uHigh: { value: new THREE.Color(TH.high) },
      uHot: { value: new THREE.Color(TH.hot) },
...
function buildEqualizer(): void {
  ...
  tint(mat, 'rings', 'uCol'); tint(mat, 'hot', 'uHot');
...
function buildAstrolabe(): void {
  ...
  CFG.rings.forEach((spec, i) => {
    ...
    for (let k = 0; k < spec.count; k++) seeds[k] = Math.random();
    ...
    g.add(mesh); bladeMeshes.push(mesh);
    ...
    const key: keyof Theme = i % 2 ? 'ringAlt' : 'rings';
    const line = new THREE.Line(new THREE.BufferGeometry().setFromPoints(pts),
      tint(new THREE.LineDashedMaterial({ color: TH[key] as number, transparent: true,
...
function buildSparks(): void {
  CFG.rings.forEach((spec, i) => {
    ...
    for (let k = 0; k < n; k++) {
      ...
      for (let q = 0; q < tl; q++) { const idx = k * tl + q; aA[idx] = a0; aSpd[idx] = sp; aOff[idx] = q; }
    }
    const geo = new THREE.BufferGeometry();
...
function buildArcs(): void {
  ...
  arcGeo.setAttribute('position',
    new THREE.BufferAttribute(new Float32Array(CFG.arcCount * CFG.arcSegs * 2 * 3), 3));
...
let arcClock = 0;
function regenArcs(): void {
  const p = arcGeo.attributes.position.array as Float32Array; let k = 0;
...
function buildStrip(): void {
  ...
  tex.wrapS = THREE.RepeatWrapping; tex.repeat.x = 2;
  strip = new THREE.Mesh(new THREE.CylinderGeometry(5.75, 5.75, 0.26, 96, 1, true),
    tint(new THREE.MeshBasicMaterial({ map: tex, color: TH.high, transparent: true, opacity: 0.24,
      blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide }), 'high'));
  strip.rotation.x = Math.PI / 2 + 0.08;
  scene.add(strip);
}

function buildScanDisc(): void {
...
function fireScan(): void { scanT = 0; }
...
function buildWaves(): void {
  for (let i = 0; i < 3; i++) {
    ...
    m.visible = false; m.userData.t = 0; m.userData.amp = 1;
    scene.add(m); waves.push(m);
  }
}
function fireWave(amp: number): void {
  const w = waves.find(x => !x.visible) || waves[0];
  w.visible = true; w.userData.t = 0; w.userData.amp = amp;
...
function buildConstellation(): void {
  ...
  for (let i = 0; i < n; i++) {
    const r = 6.6 + Math.random() * 4.0, th = Math.random() * Math.PI * 2,
  ...
  const pairs: number[] = [], slack = CFG.linkDist + CFG.linkSlack;
...
let linkCount = 0;
function updateLinks(t: number, boost: number): void {
  const pos = constel.geometry.attributes.position.array as Float32Array;
  const lp = linkGeo.attributes.position.array as Float32Array;
  const lc = linkGeo.attributes.color.array as Float32Array;
...
function updateGlyphs(t: number): void {
  const src = constel.geometry.attributes.position.array as Float32Array;
  const dst = glyphs.geometry.attributes.position.array as Float32Array;
...
function buildDust(): void {
  ...
  for (let i = 0; i < n; i++) {
    const r = 13 + Math.random() * 20, th = Math.random() * Math.PI * 2,
      ph = Math.acos(2 * Math.random() - 1);
...
function postFX(): void {
  ...
  bloomPass = new THREE.UnrealBloomPass(new THREE.Vector2(innerWidth / 2, innerHeight / 2),
...
function applyTheme(i: number, quiet?: boolean): void {
  TH = THEMES[i];
  ...
  document.querySelectorAll('#themes button').forEach(b =>
    b.setAttribute('aria-pressed', String(+(b as HTMLElement).dataset.th! === i)));
  if (!quiet) toast('espectro ' + TH.name);
}
...
function setView(i: number, quiet?: boolean): void {
  ...
  if (!quiet) toast('enquadramento ' + v.name);
}
...
function bind(): void {
  ...
  addEventListener('drop', e => {
    ...
    if (f && /audio|video|ogg|mpeg/.test(f.type)) loadTrack(f);
    else if (f) toast('formato não reconhecido');
  });
  ...
  bs.onclick = () => {
    spinning = !spinning; bs.setAttribute('aria-pressed', String(spinning)); touched();
  };
  document.getElementById('bShot')!.onclick = savePNG;
  document.querySelectorAll('#themes button').forEach(b =>
    (b as HTMLElement).onclick = () => { applyTheme(+(b as HTMLElement).dataset.th!); touched(); });

  addEventListener('keydown', e => {
    const k = e.key.toLowerCase();
    if (k === ' ') { e.preventDefault(); pulse(700); }
    else if (k === 's') fireScan();
    else if (k === 'x') toggleOpen();
    else if (k === 'v') setView(viewIdx + 1);
    else if (k === 'r') (bs as HTMLElement).click();
...
function loadTrack(f: File): void {
  audio.playFile(f).then(name => {
    markAudioButtons(); toast('a tocar · ' + name);
  }).catch(() => toast('não foi possível ler o ficheiro'));
}
function clickCore(e: MouseEvent): void {
  ndc.x = (e.clientX / innerWidth) * 2 - 1;
  ndc.y = -(e.clientY / innerHeight) * 2 + 1;
  ray.setFromCamera(ndc, camera);
  if (ray.intersectObject(hitSphere).length) pulse(900);
}
...
function animate(): void {
  ...
  if (audio.beat) { pulseUntil = performance.now() + 260; fireWave(0.55 + A.bass * 0.7); }
  ...
  if (!booted && bootP >= 1) { booted = true; fireWave(1); fireScan(); }
  ...
  arcClock += dt;
  if (arcClock > CFG.arcLife * (1 - A.level * 0.5)) { arcClock = 0; regenArcs(); }
  ...
  if (spinning) {
    ...
    ringGroups.forEach(g => {
      ...
      g.rotation.x += dt * s.x * sp; g.rotation.y += dt * s.y * sp; g.rotation.z += dt * s.z * sp;
    });
  ...
  const cp = constel.geometry.attributes.position.array as Float32Array, n = CFG.constelPts;
  for (let i = 0; i < n; i++) {
    const ix = i * 3;
  ...
  /* câmara */
  if (!cinema && t - lastInput > CFG.idleCinema) cinema = true;
  if (t - hudTouch > CFG.hudIdle) document.getElementById('hud')!.classList.add('idle');
  ...
  if (perf.acc > 1) {
    ...
    if (perf.low >= 3 && perf.quality > 0) {
      ...
      if (perf.quality === 1) { CFG.linkDist = perf.linkD; CFG.arcCount = perf.arcs; dust.visible = true; }
