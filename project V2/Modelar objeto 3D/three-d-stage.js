...
(() => {
  const stylesheet = `
    ...
    .toolbar button {
      appearance: none;
      border: 1px solid var(--stage-btn-border, rgba(20, 20, 19, 0.18));
      border-radius: var(--stage-btn-radius, 8px);
      background: var(--stage-btn-bg, rgba(255, 255, 255, 0.92));
      color: var(--stage-btn-fg, #1a1915);
      font-family: var(--stage-btn-font, inherit);
      letter-spacing: var(--stage-btn-tracking, 0);
      text-transform: var(--stage-btn-transform, none);
      font-size: var(--stage-btn-size, 12.5px);
      font-weight: 500;
      line-height: 1;
      padding: 9px 12px;
      cursor: default;
    }
    .toolbar button:hover { background: var(--stage-btn-bg-hover, #fff); }
    ...
    .note {
      ...
      color: var(--stage-note-fg, rgba(26, 25, 21, 0.55));
      letter-spacing: var(--stage-note-tracking, 0);
      user-select: none;
    }
    .err {
      position: absolute;
  ...
  class ThreeDStage extends HTMLElement {
    ...
    async _boot() {
      ...
      if (hint !== null) this.shadowRoot.querySelector('.note').textContent = hint;
      ...
      // `env-dark`: dim, warm studio over a procedural dark environment —
      // lets metals reflect something instead of reading black.
      const dark = this.hasAttribute('env-dark');
      if (dark) {
        renderer.toneMapping = THREE.ACESFilmicToneMapping;
        renderer.toneMappingExposure = 1.05;
        const envTex = this._darkEnvTexture(THREE);
        const pmrem = new THREE.PMREMGenerator(renderer);
        scene.environment = pmrem.fromEquirectangular(envTex).texture;
        envTex.dispose();
        pmrem.dispose();
      }
      scene.add(new THREE.HemisphereLight(0xffffff, 0xd8d2c4, dark ? 0.18 : 1.0));
      const key = new THREE.DirectionalLight(dark ? 0xfff0d6 : 0xffffff, dark ? 1.5 : 2.2);
      ...
      this._key = key;
      scene.add(key);
      const fill = new THREE.DirectionalLight(0xfff4e6, dark ? 0.35 : 0.5);
      fill.position.set(-5, 3, -4);
      scene.add(fill);
      if (dark) {
        const rim = new THREE.DirectionalLight(0xffc978, 0.9);
      ...
      const ground = new THREE.Mesh(
        ...
        new THREE.ShadowMaterial({ opacity: dark ? 0.42 : 0.18 })
      );
      ...
      this._loop = () => {
        ...
        if (this.onFrame) {
          try {
            this.onFrame(clock.elapsedTime, dt);
          } catch (e) {
            console.error(e);
          }
        }
    ...
    /* Live scene handles — for page-side overlays (label projection,
       camera buttons) and per-frame animation via `stage.onFrame`. */
    ...
    /** Equirectangular dark-studio gradient: a warm overhead band, two soft
     *  side highlights, a cool floor. Drawn in canvas, no external HDR. */
    _darkEnvTexture(THREE) {
      const c = document.createElement('canvas');
      c.width = 512;
      c.height = 256;
      const g = c.getContext('2d');
      const base = g.createLinearGradient(0, 0, 0, 256);
      base.addColorStop(0, '#23201c');
      base.addColorStop(0.45, '#0d0d12');
      base.addColorStop(0.55, '#08080c');
      base.addColorStop(1, '#141118');
      g.fillStyle = base;
      ...
      blob(150, 40, 150, 70, 'rgba(255,226,175,0.85)');
      blob(400, 70, 110, 60, 'rgba(255,175,90,0.5)');
      blob(60, 150, 90, 90, 'rgba(150,170,220,0.22)');
      blob(300, 200, 160, 60, 'rgba(255,200,140,0.12)');
