 ...
 *
 * ── Notas da migração para TypeScript ───────────────────────────────────────
 * O MediaPipe entra por <script> de CDN e não traz tipos. As interfaces abaixo
 * declaram **só o que esta página usa** — `FaceMesh`, `Hands` e a forma dos
 * resultados. Tipar a biblioteca inteira seria inventar contrato de código de
 * terceiro; tipar o que se consome é descrever a dependência que já existe.
 *
 * `getContext('2d')` devolve `null` em ambiente sem canvas 2D. O original
 * assumia não-nulo; aqui a ausência falha alto e cedo (`exigirCtx`) em vez de
 * estourar num `null.drawImage` dentro do laço de animação, onde a causa some.
 *
 * `window.Hands` também era assumido presente após o `loadScript`. Agora é
 * checado: script que carrega e não define o global é falha silenciosa clássica
 * de CDN, e vira mensagem em vez de `TypeError`.
 */
...
/** Um ponto normalizado (0..1) devolvido pelo MediaPipe. */
interface Landmark { readonly x: number; readonly y: number; readonly z?: number }

interface FaceMeshResults {
  readonly image: CanvasImageSource;
  readonly multiFaceLandmarks?: readonly (readonly Landmark[])[];
}

interface HandsResults {
  readonly multiHandLandmarks?: readonly (readonly Landmark[])[];
}

interface MediaPipeOpcoes { locateFile: (arquivo: string) => string }

interface FaceMeshInstancia {
...
/**
 * O contexto 2D, ou erro com a causa dita.
 *
 * Canvas sem contexto 2D é ambiente quebrado, não caso de uso — e descobrir isso
 * na construção é muito melhor do que descobrir num `null.drawImage` a 60 fps.
 */
function exigirCtx(
  canvas: HTMLCanvasElement,
  opcoes?: CanvasRenderingContext2DSettings,
...
type LadoMao = 'LEFT' | 'RIGHT';

function telaX(wristX: number): number { return 1 - wristX; }

function decidirLadoPosicional(wristX: number): LadoMao {
  return telaX(wristX) < 0.5 ? 'RIGHT' : 'LEFT';
}

function rotularMaos(maos: readonly { wristX: number }[]): LadoMao[] {
  if (maos.length === 2) {
...
type AoAlertar = (proporcao: number) => void;
...
class MotionDetector {
  ...
  sensitivity: number;
  readonly onAlert: AoAlertar | undefined;
  ...
  constructor(
    ...
    this.ctx = exigirCtx(canvas, { willReadFrequently: true });
    this.octx = exigirCtx(overlay);
  }
  ...
  private _diff(a: ImageData, b: ImageData): ImageData {
  ...
  private _drawMotion(diff: ImageData): void {
...
function loadMediaPipe(): Promise<FaceMeshCtor | null> {
  return new Promise((resolve) => {
    ...
    s.onload = () => {
      ...
      s2.onload = () => resolve(window.FaceMesh ?? null);
...
class GazeTracker {
  ...
  private readonly ctx: CanvasRenderingContext2D;
  private faceMesh: FaceMeshInstancia | null = null;
  private running = false;
  private stream: MediaStream | null = null;
  ...
  private _onResults(results: FaceMeshResults): void {
    const ctx = this.ctx;
...
class HandTracker {
  ...
  private hands: HandsInstancia | null = null;
  private stream: MediaStream | null = null;
  ...
  async start(canvas: HTMLCanvasElement): Promise<boolean> {
    ...
    /* Script que carrega e não define o global é falha silenciosa clássica de
     * CDN. O original ia direto ao `new window.Hands(...)` e estourava. */
    const Hands = window.Hands;
    if (!Hands) {
      if (this.statusEl) this.statusEl.textContent = 'MediaPipe Hands carregou, mas não se registrou.';
      return false;
    }
    try {
  ...
  private _drawHUD(ctx: CanvasRenderingContext2D, W: number, H: number): void {
    const c = 'rgba(212,162,78,0.45)';
    const len = 22;
    ctx.strokeStyle = c; ctx.lineWidth = 2; ctx.shadowBlur = 0;
    const cantos: readonly (readonly [number, number, number, number])[] = [
  ...
  stop(): void {
    ...
    if (this.raf) cancelAnimationFrame(this.raf);
    if (this.stream) this.stream.getTracks().forEach((t) => t.stop());
    if (this.hands) { try { this.hands.close(); } catch { /* já fechado */ } }
...
type ModoFiltro = 'night' | 'thermal' | 'clarity';

class VisionFilter {
  readonly mode: ModoFiltro;
  private running = false;
  private raf: number | null = null;
  private stream: MediaStream | null = null;
  private video: HTMLVideoElement | null = null;
  private canvas: HTMLCanvasElement | null = null;
  private ctx: CanvasRenderingContext2D | null = null;
  private readonly _lut: Uint8Array;

  constructor(mode: ModoFiltro) {
    this.mode = mode;
    this._lut = this._buildThermalLut();
  }

  private _buildThermalLut(): Uint8Array {
...
type Aba = 'motion' | 'gaze' | 'hand' | 'night' | 'thermal' | 'clarity';

let _motionDetector: MotionDetector | null = null;
let _gazeTracker: GazeTracker | null = null;
let _handTracker: HandTracker | null = null;
let _nightFilter: VisionFilter | null = null;
let _thermalFilter: VisionFilter | null = null;
let _clarityFilter: VisionFilter | null = null;
...
export function visaoPage(): HTMLElement {
  cleanup();

  /* ── Tab state ── */
  let activeTab: Aba = 'motion';
  ...
  /* ── Tabs ── */
  const tabs: readonly Aba[] = ['motion', 'gaze', 'hand', 'night', 'thermal', 'clarity'];
  const tabLabels: Record<Aba, string> = {
    motion: '◉ Movimento',
    gaze: '👁 Olhar',
    hand: '🤖 Mão JARVIS',
    night: '🌙 Noturno',
  ...
  const contents: Record<Aba, HTMLElement> = {
    motion: motionContent, gaze: gazeContent, hand: handContent,
    night: nightContent, thermal: thermalContent, clarity: clarityContent,
  };

  const tabBar = h('div', { className: 'visao-tabs' });
  const contentWrap = h('div', { className: 'visao-content' }, motionContent);

  const tabBtns = {} as Record<Aba, HTMLButtonElement>;
  for (const t of tabs) {
    const btn = h('button', {
