...
const MAPA: readonly MapBlock[] = [
  {
    group: 'Comando',
    items: [
      { icon: '⬡', name: 'Ponte de Comando', desc: 'A tela inicial: status do sistema, métricas e acesso rápido a tudo.' },
      { icon: '⚙', name: 'Hub de Ferramentas', desc: 'Catálogo central com mais de 35 ferramentas técnicas, busca e filtro por categoria.' }
    ]
  },
  {
    group: 'Ferramentas técnicas',
    items: [
      { icon: '⌨', name: 'Editor de Código (IDE)', desc: 'Editor multi-arquivo com destaque de sintaxe para 26 linguagens, execução de JS/HTML/CSS e um filesystem virtual.' },
      { icon: '▶', name: 'Terminal Web', desc: '60+ comandos POSIX-like, filesystem virtual persistente, pipes e histórico navegável.' },
      { icon: '∑', name: 'Calculadoras', desc: 'Científica, numérica (bin/hex/oct + IEEE 754), financeira, estatística, engenharia, saúde e conversores de unidades.' },
      { icon: '⚿', name: 'Lab de Criptografia', desc: 'César, Vigenère, Atbash, Base64/32/Hex, hashes SHA, AES-GCM e One-Time Pad — sempre com a teoria por trás.' },
      { icon: '⊨', name: 'Tabela Verdade & Regex', desc: 'Parser de lógica booleana com mapa de Karnaugh e um tester de expressões regulares explicado.' },
      { icon: '◢', name: 'Gráficos, FFT e Símbolos', desc: 'Gerador de gráficos em Canvas puro, visualizador de espectro de áudio e 1200+ símbolos Unicode.' }
    ]
  },
  {
    group: 'Conhecimento & narrativa',
    items: [
      { icon: '◫', name: 'Biblioteca — Crônicas da Baluarte', desc: 'A fan fic "Onde os Deuses Sangram" completa: 24 arcos e mais de mil capítulos, com leitor, favoritos e retomada de leitura.' },
      { icon: '◬', name: 'Academia', desc: 'Tutoriais de linguagens de programação, do primeiro passo ao avançado.' },
      { icon: '✦', name: 'Universo', desc: 'As wikis dos mundos cruzados pelas Crônicas: facções, ameaças e linha do tempo.' },
      { icon: '⚛', name: 'Referência', desc: 'Tabela periódica dos 118 elementos, catálogo de mods, guia para montar PC e um simulador de lógica digital.' }
    ]
  },
  {
    group: 'Tático & sistema',
    items: [
      { icon: '◆', name: 'Elites & Arsenal', desc: 'As equipes operacionais ALFA → ZULU e o catálogo de armas, veículos e doutrinas táticas.' },
      { icon: '⚿', name: 'CiberSeg', desc: 'Enciclopédia de segurança cibernética: conceitos, vetores de ataque e defesas.' },
      { icon: '◈', name: 'Economia', desc: 'Cotações de câmbio e criptomoedas em tempo real, com conversor embutido.' },
      { icon: '◉', name: 'J.A.R.V.I.S. & IA Mark 11', desc: 'O assistente de IA com memória, e a IA Proprietária — um sistema de Skills modulares.' }
    ]
  }
];

const EDUCACIONAL: readonly EducationalItem[] = [
  { icon: '⚿', text: 'O Lab de Criptografia não só cifra textos — explica César, Vigenère, análise de frequência, funções hash e a segurança provada do One-Time Pad.' },
  { icon: '⊨', text: 'A Tabela Verdade ensina lógica booleana de verdade: AND, OR, NOT, XOR, mapas de Karnaugh e simplificação de circuitos.' },
  { icon: '⚛', text: 'A Tabela Periódica e o simulador de lógica digital transformam química e eletrônica em algo que dá para tocar, montar e testar.' },
  { icon: '⌨', text: 'Editor e Terminal são um ambiente seguro para escrever, rodar e quebrar código — aprender programando, sem medo de errar.' },
  { icon: '◫', text: 'Até o software por trás é uma aula: este site é um estudo de caso de engenharia incremental — 21 fases, cada uma testada antes da seguinte.' }
];

export function sobrePage(args: PageArgs = {}): HTMLDivElement {
  ...
  const timeline = h('div', { className: 'sobre-timeline' });
  ...
  page.appendChild(
    h('div', { className: 'sobre-build' },
      ...
      h('p', null,
        ...
        '. O Baluarte continua sendo construído, e ainda vai ganhar muito mais conteúdo. É para esperar novas versões depois desta.'),
      h('p', null,
