...
import type { A3Arma } from '../data/arma3-armas.js';
...
/** Um ponto geográfico da tela: lat/lon em graus, altitude em metros. */
interface PontoTela { readonly lat: number; readonly lon: number; readonly alt: number }

/** Só lat/lon — o que o clique no mapa produz, antes de ganhar altitude. */
interface LatLonTela { readonly lat: number; readonly lon: number }

/**
 * Ponte mapa → computador de tiro.
 *
 * Os dois cards vivem neste módulo; o `cardComputador` registra `aplicar` e o
 * mapa chama quando o operador pede "levar para o computador". Sem evento
 * global, sem estado fora da página — por isso `aplicar` é opcional: existe a
 * janela entre montar o mapa e montar o computador em que ainda não há ponte.
 */
interface PonteComputador {
  aplicar?: (peca: PontoTela, alvo: PontoTela) => void;
}
...
const PADRAO: { readonly peca: PontoTela; readonly alvo: PontoTela } = {
  peca: { lat: 39.8000, lon: 25.0000, alt: 20 },
  alvo: { lat: 39.8100, lon: 25.0150, alt: 65 },
};

const nUm = (v: number | null | undefined, casas = 0): string =>
  (typeof v === 'number' && Number.isFinite(v) ? v.toFixed(casas) : '—');

const ponteComputador: PonteComputador = {};

/** Lê o valor de um input a partir do evento, sem supor o alvo do handler. */
function valorDoCampo(e: Event): string {
  const alvo = e.target;
  return alvo instanceof HTMLInputElement || alvo instanceof HTMLSelectElement
    ? alvo.value
    : '';
}

interface VanguardPageArgs { query?: Readonly<Record<string, string>> | null }

export function vanguardPage(args: VanguardPageArgs = {}): HTMLDivElement {
 ...
 */
function cardMapaTatico(): HTMLDivElement {
  let modo: 'peca' | 'alvo' | null = null;
  let peca: LatLonTela | null = null;
  let alvo: LatLonTela | null = null;
  let altPeca = 0, altAlvo = 0;
  let mapa: MapLibreMap | null = null;
  let mkPeca: MapLibreMarker | null = null, mkAlvo: MapLibreMarker | null = null;
  ...
  const fmt = (p: LatLonTela): string => `${p.lat.toFixed(6)}, ${p.lon.toFixed(6)}`;

  function leituraPonto(
    el: HTMLDivElement, rotulo: string, p: LatLonTela | null, alt: number,
  ): void {
    el.replaceChildren(
  ...
  function recalcVetor(): void {
    ...
    /* as duas posições entram em const: o narrowing acima não sobrevive ao
     * callback do botão, e são elas que vão para o computador de tiro */
    const p = peca, a = alvo;
    const v = gridVector({ ...p, alt: altPeca }, { ...a, alt: altAlvo });
        ...
        onclick: () => {
          if (ponteComputador.aplicar) {
            ponteComputador.aplicar(
              { lat: p.lat, lon: p.lon, alt: altPeca },
              { lat: a.lat, lon: a.lon, alt: altAlvo });
          }
        },
      }, '▶ Levar para o computador de tiro'));
  }

  function marcar(ml: MapLibreNamespace, lngLat: MapLibreLngLat): void {
    if (!mapa) return;
    const p: LatLonTela = { lat: lngLat.lat, lon: lngLat.lng };
    if (modo === 'peca') {
      peca = p;
      if (!mkPeca) {
  ...
  const btnModo = (id: 'peca' | 'alvo', rotulo: string): HTMLButtonElement => {
    const b = h('button', {
      className: 'btn vg-mapa__modo', dataset: { modo: id },
      onclick: () => {
        modo = modo === id ? null : id;
        barra.querySelectorAll<HTMLElement>('.vg-mapa__modo').forEach((x) =>
  ...
  const altIn = (valor: number, set: (v: number) => void): HTMLInputElement => h('input', {
    className: 'input vg-num-in vg-mapa__alt', type: 'number',
    value: String(valor), step: '1', min: '-500', max: '9000',
    oninput: (e: Event) => {
      const x = parseFloat(valorDoCampo(e));
 ...
 */
function cardTerreno(terrenoPedido: string | undefined): HTMLDivElement {
  const terrenos = [...A3TER].filter((t) => t.grade)
    .sort((a, b) => (Number(a.ehMod) - Number(b.ehMod))
  ...
  /* o primeiro da lista é o fallback e a lista não é vazia (checado acima) */
  const inicial = terrenos.find((t) => t.id === terrenoPedido)
    || terrenos.find((t) => t.classe === 'Altis') || terrenos[0];
  let terrenoId = inicial.id;
  let armaId = '';
  let zero = 300;
  ...
  function gradesIniciais(t: A3Terreno): { a: string; b: string } {
    const g = t.grade;
  ...
  const ini = gradesIniciais(inicial);
  let gradePeca = ini.a;
  let gradeAlvo = ini.b;

  const saida = h('div', null);

  function terrenoAtual(): A3Terreno {
    return terrenos.find((x) => x.id === terrenoId) || terrenos[0];
  }
  ...
  function recalc(): void {
    ...
    const linhaAz = (rotulo: string, grau: number): HTMLTableRowElement =>
      h('tr', { className: 'vg-tr' },
        h('td', null, rotulo),
    ...
    saida.replaceChildren(...filhos.filter((f): f is Node => f instanceof Node));
  }

  const selTerreno = h('select', {
    className: 'input', onchange: (e: Event) => {
      terrenoId = valorDoCampo(e);
      const novo = gradesIniciais(terrenoAtual());
    ...
    + `${t.dlc}${t.tamanhoM ? ` · ${(t.tamanhoM / 1000).toFixed(0)} km` : ''}`)));

  const inGrade = (valor: string, set: (v: string) => void): HTMLInputElement => h('input', {
    className: 'input vg-num-in', type: 'text', value: valor,
    placeholder: '034056', spellcheck: 'false',
    oninput: (e: Event) => { set(valorDoCampo(e)); recalc(); },
  });
  const inPeca = inGrade(gradePeca, (v) => { gradePeca = v; });
  const inAlvo = inGrade(gradeAlvo, (v) => { gradeAlvo = v; });

  const selArma = h('select', {
    className: 'input', onchange: (e: Event) => { armaId = valorDoCampo(e); recalc(); },
...
function cardMrad(): HTMLDivElement {
  const armas = A3ARM.filter((a) => dadosBalisticos(a))
    .sort((a, b) => a.nome.localeCompare(b.nome, 'pt-BR'));
  ...
  function recalc(): void {
    const a: A3Arma = armas.find((x) => x.id === armaId) || armas[0];
    const bal = dadosBalisticos(a);
    /* A lista já foi filtrada por `dadosBalisticos(a)`, mas o filtro não
     * estreita o tipo do elemento — sem esta guarda a tabela leria initSpeed
     * de um `null`, que é o defeito que a filtragem existe pra impedir. */
    if (!bal) { saida.replaceChildren(); return; }
      ...
      h('div', { className: 'vg-tabela-wrap' },
        h('table', { className: 'vg-tabela' },
          ...
          h('tbody', null, ...linhas.map((r) => {
            const naZeragem = Math.abs(r.d - zero) < 1;
            ...
            const corr = -r.mils;
            return h('tr', { className: 'vg-tr' + (naZeragem ? ' is-preferida' : '') },
              h('td', null, `${r.d} m`, naZeragem ? h('span', { className: 'vg-tag' }, 'zerada') : null),
  ...
  const numIn = (
    v: number, min: number, max: number, passo: number, set: (x: number) => void,
...
/** As chaves dos campos numéricos — o mapa de inputs é indexado por elas. */
type CampoComputador =
  | 'pecaLat' | 'pecaLon' | 'pecaAlt'
  | 'alvoLat' | 'alvoLon' | 'alvoAlt'
  | 'ventoVel' | 'ventoDir';

function cardComputador(): HTMLDivElement {
  const sistemas = listarSistemas();
  let sistemaId = sistemas[0].id;
  const campos: Record<CampoComputador, number> = {
  ...
  function calcular(): void {
    ...
    try {
      ...
      saida.replaceChildren(h('p', { className: 'vg-erro' }, `Erro no motor: ${msg}`));
      return;
    }

    /* `erros` só existe na resposta recusada na validação. A resposta
     * resolvida também pode vir com `ok: false` — quando nenhuma carga
     * alcança — e aí quem explica é `avisos`, carga por carga. */
    if ('erros' in r) {
    ...
    const resolvida: MissaoRespostaResolvida = r;
    const g = resolvida.geometria, az = resolvida.azimute;
    const preferida: SolucaoCarga | undefined =
      resolvida.solucoes.find((s) => s.preferida) || resolvida.solucoes[0];

    /* ⚠️ Os `null` condicionais abaixo NÃO podem ir direto para o
     * `replaceChildren`: diferente do `h()`, que descarta filho nulo, o
     * `replaceChildren` converte o que não é Node em texto — `null` vira a
     * palavra "null" na tela. Medido no Chromium:
     * `d.replaceChildren(p, null, null)` → `"<p>ALGO</p>nullnull"`.
     * Por isso a lista é montada e FILTRADA antes de entrar. */
    const blocos: HChild[] = [
      h('div', { className: 'vg-grid' },
      ...
      resolvida.solucoes.length
        ? h('div', { className: 'vg-tabela-wrap' },
          h('table', { className: 'vg-tabela' },
            ...
            h('tbody', null, ...resolvida.solucoes.map((s) => h('tr', {
      ...
      resolvida.avisos && resolvida.avisos.length
        ? h('ul', { className: 'vg-avisos' }, ...resolvida.avisos.map((a) => h('li', null, `⚠ ${a}`)))
        : null,
    ];
    saida.replaceChildren(...blocos.filter((b): b is Node => b instanceof Node));
  }

  /* Guardados por chave pra que o mapa tático consiga reescrever os campos —
   * ver ponteComputador.aplicar abaixo. */
  const entradas = new Map<CampoComputador, HTMLInputElement>();
  const num = (
    chave: CampoComputador, min: number, max: number, passo: number,
  ): HTMLInputElement => {
    const el = h('input', {
      className: 'input vg-num-in', type: 'number', value: String(campos[chave]),
      min: String(min), max: String(max), step: String(passo),
      oninput: (e: Event) => {
        const v = parseFloat(valorDoCampo(e));
        if (Number.isFinite(v)) { campos[chave] = v; calcular(); }
      },
    });
    entradas.set(chave, el);
    return el;
  };

  const selSis = h('select', {
    className: 'input', onchange: (e: Event) => { sistemaId = valorDoCampo(e); calcular(); }
  ...
  ponteComputador.aplicar = (p, a) => {
    ...
    entradas.forEach((el, k) => { el.value = String(campos[k]); });
    calcular();
...
function leitura(rot: string, valor: HChild, sub: string): HTMLDivElement {
...
function cardCoordenadas(): HTMLDivElement {
  ...
  const entrada = h('input', {
    ...
    oninput: (e: Event) => converter(valorDoCampo(e)),
  });

  function converter(texto: string): void {
    const t = (texto || '').trim();
    ...
    if (campos.length === 2 && campos.every((c) => c !== '' && Number.isFinite(Number(c)))) {
      ...
      try {
        ...
        saida.replaceChildren(h('p', { className: 'vg-erro' },
          `Não reconheci "${t}" — use "lat, lon" ou uma referência MGRS. (${msg})`));
        return;
      }
    }
...
/* ═══════════ o que é e onde mora ═══════════ */
function cardArquitetura(): HTMLDivElement {
  const item = (t: string, d: string): HTMLLIElement =>
