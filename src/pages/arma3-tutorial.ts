...
/* Só os CONTADORES (1 kB): a barra de abas precisa dos totais, e lê-los das
 * bases obrigava a carregar 2,2 MB antes de desenhar um rótulo. As bases em si
 * descem sob demanda — ver `BASES`/`carregarAba` abaixo. */
import {
  A3ARM_TOTAL, A3ACC_TOTAL, A3VEI_TOTAL, A3EQP_TOTAL, A3SOL_TOTAL, A3TER_TOTAL,
  A3MUN_TOTAL, A3MAG_TOTAL, A3COL_TOTAL, A3TUT_TOTAL
} from '../data/arma3-totais.js';
...
import type { A3ClsContexto } from '../data/arma3-classes.js';
import { resolverTiro, dadosBalisticos } from '../utils/arma3-balistica.js';
import type { ResultadoTiro } from '../utils/arma3-balistica.js';
import type { A3Arma, A3ArmaTipo } from '../data/arma3-armas.js';
import type { A3Carregador, A3Municao } from '../data/arma3-municao.js';
import type { A3Acessorio } from '../data/arma3-acessorios.js';
import type { A3Terreno } from '../data/arma3-terrenos.js';
import type { A3Veiculo } from '../data/arma3-veiculos.js';
import type { A3Equipamento } from '../data/arma3-equipamento.js';
import type { A3Soldado } from '../data/arma3-soldados.js';
import type { A3ColItem } from '../data/arma3-colecao.js';
import type { A3TutMod } from '../data/arma3-tutoriais.js';

const PRESET_ID = 'projeto-baluarte-vercel-app';

/* Roda dentro do Baluarte Launcher? A ponte só existe no app.
 *
 * `window.baluarte` já é declarado globalmente por `arma3-extracao.ts` e
 * `jarvis-hermes-native.ts` — redeclarar aqui daria `TS2717`, e a ponte é uma
 * só no site inteiro. */
 ...
 * estaticamente custava 2,2 MB de JavaScript na abertura — para desenhar a aba
 * padrão, que precisa de 33 kB. Era o maior peso do site.
 *
 * Agora cada base desce quando a aba dela é aberta. `D` é o depósito do que já
 * chegou; as funções de render leem de `D`, nunca de um import.
 *
 * Os TOTAIS não moram aqui: eles vêm de `arma3-totais.js` (1 kB), porque a
 * barra de abas mostra o número de todas as abas desde o primeiro quadro.
 */

/**
 * O depósito, tipado como a interseção dos módulos que podem cair nele —
 * cada um `Partial`, porque "ainda não chegou" é o estado normal.
 *
 * Isto é o que impede a tela de ler `D.A3ARM` como se estivesse garantido: o
 * tipo exige passar por `exigirBase()`, que falha alto em vez de deixar um
 * `undefined.filter` estourar cinco quadros depois, longe da causa.
 */
type Deposito =
  Partial<typeof import('../data/arma3-armas.js')>
  & Partial<typeof import('../data/arma3-municao.js')>
  & Partial<typeof import('../data/arma3-acessorios.js')>
  & Partial<typeof import('../data/arma3-terrenos.js')>
  & Partial<typeof import('../data/arma3-veiculos.js')>
  & Partial<typeof import('../data/arma3-equipamento.js')>
  & Partial<typeof import('../data/arma3-soldados.js')>
  & Partial<typeof import('../data/arma3-colecao.js')>
  & Partial<typeof import('../data/arma3-tutoriais.js')>;

const D: Deposito = {};

/** Uma aba que tem base própria para descer. */
type AbaComBase =
  | 'armas' | 'municao' | 'carregadores' | 'acessorios' | 'terrenos'
  | 'veiculos' | 'equipamento' | 'soldados' | 'colecao' | 'mods';

type Carregador = () => Promise<unknown>;

const CARREGADORES: Readonly<Record<AbaComBase, Carregador>> = {
  armas: () => import('../data/arma3-armas.js'),
  municao: () => import('../data/arma3-municao.js'),
  carregadores: () => import('../data/arma3-municao.js'),   // carregadores saem da mesma base
  acessorios: () => import('../data/arma3-acessorios.js'),
  terrenos: () => import('../data/arma3-terrenos.js'),
  veiculos: () => import('../data/arma3-veiculos.js'),
  equipamento: () => import('../data/arma3-equipamento.js'),
  soldados: () => import('../data/arma3-soldados.js'),
  colecao: () => import('../data/arma3-colecao.js'),
  mods: () => import('../data/arma3-tutoriais.js')
};

function temCarregador(aba: string): aba is AbaComBase {
  return Object.prototype.hasOwnProperty.call(CARREGADORES, aba);
}

/* A aba de mods cita a coleção no cabeçalho, então pede as duas bases.
 * Carregar a segunda é barato e evita render pela metade. */
const EXTRA: Readonly<Record<string, readonly AbaComBase[] | undefined>> = { mods: ['colecao'] };

/* Um export por base, para saber se ela já está em `D` sem repetir a lista de
 * carregadores. Serve só para decidir se vale mostrar "carregando". */
const PRIMEIRA_CHAVE: Readonly<Record<AbaComBase, keyof Deposito>> = {
  armas: 'A3ARM', municao: 'A3MUN', carregadores: 'A3MAG', acessorios: 'A3ACC',
  terrenos: 'A3TER', veiculos: 'A3VEI', equipamento: 'A3EQP', soldados: 'A3SOL',
  colecao: 'A3COL_ITENS', mods: 'A3TUT_MODS'
};

const pendente: Record<string, Promise<void> | undefined> = {};

/**
 * A base pedida, ou uma falha explícita.
 *
 * A alternativa — devolver `undefined` e seguir — daria uma tela vazia sem
 * motivo declarado, que é o retrato de peça desligada. A versão JavaScript
 * também estourava aqui (`undefined.filter`); isto só troca o erro por um que
 * diz qual base faltou.
 */
function exigirBase<K extends keyof Deposito>(chave: K): NonNullable<Deposito[K]> {
  const valor = D[chave];
  if (valor === undefined) {
    throw new Error(`arma3-tutorial: a base "${String(chave)}" ainda não carregou`);
  }
  return valor as NonNullable<Deposito[K]>;
}

/** Garante que as bases de `aba` estejam em `D`. Idempotente e sem corrida. */
async function carregarAba(aba: string): Promise<void> {
  const alvos = [aba, ...(EXTRA[aba] ?? [])].filter(temCarregador);
  await Promise.all(alvos.map((a) => {
    const emCurso = pendente[a];
    if (emCurso) return emCurso;
    const p = CARREGADORES[a]().then((mod) => { Object.assign(D, mod); });
    pendente[a] = p;
    return p;
  }));
}
...
/** O que uma coluna da tabela ordenável precisa saber fazer. */
interface Coluna<T> {
  /** Chave de ordenação. Coluna sem `k` não ordena (a da miniatura, por ex.). */
  readonly k?: string;
  readonly rot: string;
  readonly cls?: string;
  readonly celCls?: string;
  readonly descPrimeiro?: boolean;
  /** O valor para COMPARAR — separado do que aparece. */
  readonly val?: (e: T) => string | number | null | undefined;
  /** O que aparece na célula. */
  readonly cel: (e: T) => HChild;
}

interface EstadoOrdem { col: string | null; desc: boolean }

/** Um chip de categoria, comum às cinco fontes que alimentam a barra. */
interface ChipCat {
  readonly id: string;
...
/**
 * Um tópico de qualquer trilha curada.
 *
 * As cinco trilhas (vanilla, config, comandos, campanhas, drive) têm formatos
 * quase iguais e são renderizadas pelas mesmas duas funções. Este é o mínimo
 * comum: `sqf` só existe em comandos, `link` só em drive, `atalhos` falta em
 * drive e em 10 dos 42 tópicos do vanilla.
 */
interface TopicoGenerico {
  readonly titulo: string;
  readonly texto: string;
  readonly atalhos?: readonly (readonly [string, string])[];
  readonly dicas: readonly string[];
  readonly sqf?: string | null;
  readonly link?: { readonly url: string; readonly rotulo: string };
}

/** Uma seção de trilha. `topicos` em quatro delas, `itens` em comandos. */
interface SecaoGenerica {
  readonly id: string;
  readonly nome: string;
  readonly icon: string;
  readonly desc: string;
  readonly topicos?: readonly TopicoGenerico[];
  readonly itens?: readonly TopicoGenerico[];
}

interface Arma3TutorialArgs { query?: Readonly<Record<string, string>> | null }

export async function arma3TutorialPage(args: Arma3TutorialArgs = {}): Promise<HTMLDivElement> {
  /* O router já sabe esperar handler que devolve Promise (`resolve()` em
   * `core/router.js` trata `view.then`), então dá para ter as bases da aba
   * inicial em mãos antes de montar — sem tela pela metade e sem render
   * assíncrono espalhado pelo arquivo. */
  const query = args.query || {};
  const _abaPedida = query.aba;
  const _armaPedida = query.arma;
  await carregarAba(_abaPedida || (_armaPedida ? 'armas' : 'vanilla'));
  const page = h('div', { className: 'page-a3tut' });
  const preset = ARMA3_PRESETS.find((p) => p.id === PRESET_ID);
  if (!preset) {
    /* O preset oficial some do catálogo se alguém mudar o id do gerador. Sem
     * ele metade da tela é mentira (link do arquivo, URLs do Workshop), então
     * a página diz isso em vez de renderizar pela metade. */
    page.appendChild(h('div', { className: 'card a3tut-vazio u-text-muted' },
      `O preset "${PRESET_ID}" não está em arma3-presets.js — a página do tutorial depende dele.`));
    return page;
  }
  const urlPorId: Record<string, string | undefined> = Object.fromEntries(
    preset.mods.flatMap((m) => {
      const achado = /id=(\d+)/.exec(m.url);
      return achado ? [[achado[1], m.url] as const] : [];
    }));

  let catAtiva = 'all';

  /* Armas que a calculadora aceita: só as que têm um projétil balístico de
   * verdade (ver dadosBalisticos). É o mesmo filtro do seletor, do botão da
   * tabela e do contador do cabeçalho.
   *
   * Vazio enquanto a base de armas não desceu: quem abre em `vanilla` não paga
   * os 122 kB do arsenal só para a calculadora existir escondida. É recalculado
   * assim que a aba de armas carrega (`sincronizarCalculadora`). */
  let armasCalculaveis: readonly A3Arma[] = D.A3ARM ? D.A3ARM.filter((a) => dadosBalisticos(a)) : [];
  ...
  const buscaInicial = query.q || '';

  /** A arma que a calculadora abre, pela ordem de preferência de sempre. */
  function escolherArmaPadrao(): string | undefined {
    return (armasCalculaveis.find((a) => a.id === armaPedida)
      || armasCalculaveis.find((a) => a.classe === 'arifle_MX_F')
      || armasCalculaveis.find((a) => a.origem === 'Base' && a.tipo === 'fuzil')
      || armasCalculaveis[0])?.id;
  }
  let calcArmaId = escolherArmaPadrao();
  let calcZero = 300, calcAlvo = 600, calcVento = 0;

  /* Chamada depois de cada carregamento: a lista de armas calculáveis e a arma
   * escolhida dependem de uma base que agora pode chegar depois da montagem. */
  function sincronizarCalculadora(): void {
    if (!D.A3ARM || armasCalculaveis.length) return;
    armasCalculaveis = D.A3ARM.filter((a) => dadosBalisticos(a));
    calcArmaId = escolherArmaPadrao();
  }
  sincronizarCalculadora();

  /* arsenal completo (mods) — só desce quando o operador pede */
  let expandido: string | null = null;   // id da arma com os modos de tiro abertos
  let arsenalMods: readonly A3Arma[] | null = null;
  let arsenalEstado: 'ocioso' | 'carregando' | 'pronto' | 'erro' = 'ocioso';
  const abaInicial = query.aba;
  const ABAS: string[] = ['vanilla', 'armas', 'acessorios', 'veiculos', 'equipamento', 'soldados', 'terrenos', 'municao',
    'carregadores', 'colecao', 'config', 'mods', 'comandos',
    'campanhas', 'drive'];
  let aba = abaInicial && ABAS.includes(abaInicial) ? abaInicial : (armaPedida ? 'armas' : 'vanilla');
  ...
  const abaBtn = (id: string, label: string): HTMLButtonElement => h('button', {
    className: 'symbols-cat a3tut-aba' + (aba === id ? ' is-active' : ''), dataset: { aba: id },
    onclick: async () => {
      aba = id; busca = ''; buscaInput.value = ''; catAtiva = 'all';
      abas.querySelectorAll<HTMLElement>('.a3tut-aba').forEach((b) => b.classList.toggle('is-active', b.dataset.aba === id));
      /* A base da aba pode ainda não ter descido. Enquanto desce, a tela avisa
       * — melhor um "carregando" de um quadro que um corpo vazio sem motivo. */
      if (temCarregador(id) && !D[PRIMEIRA_CHAVE[id]]) {
        corpo.replaceChildren(h('p', { className: 'u-text-muted' }, 'Carregando base…'));
      }
      await carregarAba(id);
      sincronizarCalculadora();
      if (aba !== id) return;            // o operador já trocou de aba de novo
      montarChips(); render();
    }
  ...
  const buscaInput = h('input', {
    ...
    oninput: debounce((e: Event) => {
      const alvo = e.target;
      if (alvo instanceof HTMLInputElement) { busca = alvo.value; render(); }
    }, 120)
  });
  busca = buscaInicial;
  const chips = h('div', { className: 'symbols-cats' });
  function secoesDaAba(): readonly SecaoGenerica[] | null {
    ...
    return null; // mods usa D.A3TUT_CATEGORIAS · colecao usa D.A3COL_CATS
  }
  function montarChips(): void {
    ...
    const TUDO: ChipCat = { id: 'all', nome: 'Tudo', icon: '⬡' };
    const CATS_DA_ABA: Readonly<Record<string, readonly ChipCat[] | undefined>> = {
      mods: D.A3TUT_CATEGORIAS,
      colecao: D.A3COL_CATS,
      armas: D.A3ARM_TIPOS,
      veiculos: D.A3VEI_CATEGORIAS,
      equipamento: D.A3EQP_CATEGORIAS,
    };
    const daAba = CATS_DA_ABA[aba];
    let cats: readonly ChipCat[];
    if (daAba) {
      cats = [TUDO, ...daAba];
    } else if (secs) {
    ...
    cats.forEach((c) => {
      chips.appendChild(h('button', {
        ...
        onclick: () => {
          catAtiva = c.id;
          chips.querySelectorAll<HTMLElement>('.symbols-cat').forEach((b) => b.classList.toggle('is-active', b.dataset.cat === c.id));
          render();
        }
      },
  ...
  const linhaAtalho = ([tecla, acao]: readonly [string, string]): HTMLDivElement =>
    h('div', { className: 'a3tut-atalho' },
      h('kbd', { className: 'a3tut-kbd' }, tecla),
      h('span', { className: 'a3tut-atalho__acao' }, acao));

  function cardMod(id: string, t: A3TutMod): HTMLDivElement {
  ...
  function cardTopico(t: TopicoGenerico): HTMLDivElement {
    return h('div', { className: 'card a3tut-card' },
      h('div', { className: 'a3tut-card__head' },
  ...
  function cardComando(t: TopicoGenerico): HTMLDivElement {
    let sqfEl: HTMLDivElement | null = null;
    if (t.sqf) {
      /* o comando entra numa const: dentro do callback o narrowing de `t.sqf`
       * não sobrevive, e é ele que vai pro clipboard */
      const comando = t.sqf;
      const copiarBtn = h('button', {
        className: 'a3tut-sqf__copiar',
        onclick: () => {
          const done = (): void => { copiarBtn.textContent = 'copiado ✓'; setTimeout(() => { copiarBtn.textContent = '⧉ copiar'; }, 1600); };
          if (navigator.clipboard && navigator.clipboard.writeText) navigator.clipboard.writeText(comando).then(done).catch(() => {});
          else done();
        }
      }, '⧉ copiar');
      sqfEl = h('div', { className: 'a3tut-card__sec' },
        ...
        h('div', { className: 'a3tut-sqf' },
          h('pre', { className: 'a3tut-sqf__code' }, comando),
          copiarBtn));
    }
    return h('div', { className: 'card a3tut-card' },
      h('div', { className: 'a3tut-card__head' },
  ...
  function irParaTutorial(nome: string): void {
    const btn = abas.querySelector<HTMLButtonElement>('[data-aba="mods"]');
    if (btn) btn.click();
  ...
  function cardColecao(id: string, it: A3ColItem): HTMLDivElement {
  ...
  function viewerDrive(): HTMLDivElement {
    ...
    A3DRV_PASTAS.forEach((p) => {
      chipsPastas.appendChild(h('button', {
        ...
        onclick: () => {
          pastaAtiva = p;
          chipsPastas.querySelectorAll<HTMLElement>('.symbols-cat').forEach((b) => b.classList.toggle('is-active', b.dataset.pasta === p.id));
  ...
  const MOTIVO_SEM_IMG: Readonly<Record<string, string | undefined>> = {
  ...
  function thumb(a: A3Arma, tam: 'sm' | 'lg' = 'sm'): HTMLElement {
    if (a.img) {
      return h('img', {
    ...
    return h('span', {
      ...
      title: (a.imgAusente ? MOTIVO_SEM_IMG[a.imgAusente] : null) || 'sem ícone',
    }, '⃠');
  }

  /* Número do config ou marca de ausência — nunca 0 no lugar de "não sei". */
  const val = (x: number | null | undefined, sufixo = '', casas = 0): HTMLSpanElement =>
    (typeof x === 'number'
  ...
  function svgTrajetoria(res: ResultadoTiro, alvo: number): SVGSVGElement | null {
    ...
    const sx = (x: number): number => padL + (x / maxX) * (W - padL - padR);
    const sy = (y: number): number => padT + (1 - (y - yMin) / span) * (H - padT - padB);
    const d = pts.map((p, i) => `${i ? 'L' : 'M'}${sx(p.x).toFixed(1)},${sy(p.y).toFixed(1)}`).join(' ');
    const NS = 'http://www.w3.org/2000/svg';
    const el = document.createElementNS(NS, 'svg');
    el.setAttribute('viewBox', `0 0 ${W} ${H}`); el.setAttribute('class', 'a3arm-svg');
    const mk = (t: string, at: Readonly<Record<string, string | number>>): SVGElement => {
      const n = document.createElementNS(NS, t);
      for (const k in at) n.setAttribute(k, String(at[k]));
      return n;
    };
  ...
  function calcBalistica(): HTMLDivElement {
    const A3ARM = exigirBase('A3ARM');
    const A3ARM_TIPOS = exigirBase('A3ARM_TIPOS');
    ...
    function recalc(): void {
      ...
      const bal = a ? dadosBalisticos(a) : null;
      if (!a || !bal) {
    ...
    const selArma = h('select', {
      className: 'input a3arm-sel',
      onchange: (e: Event) => {
        const alvo = e.target;
        if (alvo instanceof HTMLSelectElement) { calcArmaId = alvo.value; recalc(); }
      }
    });
    ...
    const num = (
      valor: number, min: number, max: number, step: number, set: (v: number) => void,
    ): HTMLInputElement => h('input', {
      className: 'input a3arm-num', type: 'number', value: String(valor), min: String(min), max: String(max), step: String(step),
      oninput: (e: Event) => {
        const alvo = e.target;
        if (!(alvo instanceof HTMLInputElement)) return;
        const v = parseFloat(alvo.value);
        if (!isNaN(v)) { set(Math.max(min, Math.min(max, v))); recalc(); }
      }
    });
    box.append(
      h('div', { className: 'a3tut-card__head' },
  ...
  function resultado(rotulo: string, valor: string, sub: string): HTMLDivElement {
  ...
  const ordem: Record<string, EstadoOrdem | undefined> = {};
  ...
  function celClasse(classe: string, contexto: A3ClsContexto): HTMLElement {
  ...
  function tabelaSort<T>(
    idTabela: string, colunas: readonly Coluna<T>[], itens: readonly T[],
  ): HTMLDivElement {
    ...
    const th = (c: Coluna<T>): HTMLTableCellElement => h('th', {
      ...
      onclick: c.k ? () => {
        ...
        else { est.col = c.k ?? null; est.desc = !!c.descPrimeiro; }
        render();
      } : null,
  ...
  const secao = (
    icon: string, titulo: string, desc: string, qtd: number, ...corpoEls: HChild[]
  ): HTMLDivElement => h('div', { className: 'a3tut-secao' },
  ...
  function celModos(a: A3Arma): HTMLElement {
    const ms = a.modos || [];
  ...
  function detalheModos(a: A3Arma): HTMLDivElement {
    const ms = a.modos || [];
  ...
  /* ===== munições ===== */
  function cardFurtividade(): HTMLDivElement {
    const A3MUN = exigirBase('A3MUN');
    ...
    return h('div', { className: 'card a3tut-card a3arm-proc' },
      ...
      h('ul', { className: 'a3tut-dicas' },
        ...
        h('li', null, h('b', null, 'Furtividade. '),
          ...
          h('b', null, q.length ? String(q[0].audibleFire) : '—'), ' a ',
          h('b', null, q.length ? String(q[q.length - 1].audibleFire) : '—'),
  ...
  function tabelaMunicao(lista: readonly A3Municao[]): HTMLDivElement {
    const flag = (on: boolean, txt: string, cls: string): HTMLSpanElement => (on
      ? h('span', { className: `a3arm-flag ${cls}` }, txt)
      : h('span', { className: 'a3arm-td__na' }, '·'));
    const colunas: readonly Coluna<A3Municao>[] = [
   ...
   */
  function cardAcessorios(): HTMLDivElement {
    const A3ACC = exigirBase('A3ACC');
    const A3ACC_META = exigirBase('A3ACC_META');
  ...
  function tabelaAcessorios(lista: readonly A3Acessorio[]): HTMLDivElement {
    const TIPO_ROT: Readonly<Record<string, string | undefined>> = {
      mira: '🔭 mira', silenciador: '🤫 silenciador',
      apontador: '🔦 laser', bipe: '⚙️ bipé',
    };
    const colunas: readonly Coluna<A3Acessorio>[] = [
  ...
  function tabelaVeiculos(lista: readonly A3Veiculo[]): HTMLDivElement {
    const CAT = Object.fromEntries(exigirBase('A3VEI_CATEGORIAS').map((c) => [c.id, c]));
    const colunas: readonly Coluna<A3Veiculo>[] = [
  ...
  function tabelaEquipamento(lista: readonly A3Equipamento[]): HTMLDivElement {
    const CAT = Object.fromEntries(exigirBase('A3EQP_CATEGORIAS').map((c) => [c.id, c]));
    const colunas: readonly Coluna<A3Equipamento>[] = [
  ...
  function tabelaSoldados(lista: readonly A3Soldado[]): HTMLDivElement {
    const colunas: readonly Coluna<A3Soldado>[] = [
  ...
  function cardTerrenos(): HTMLDivElement {
    const A3TER = exigirBase('A3TER');
  ...
  function tabelaTerrenos(lista: readonly A3Terreno[]): HTMLDivElement {
    const colunas: readonly Coluna<A3Terreno>[] = [
  ...
  /* ===== carregadores ===== */
  function cardCarregadores(): HTMLDivElement {
    const A3MAG = exigirBase('A3MAG');
    const variam: Record<string, Set<number> | undefined> = {};
    A3MAG.forEach((m) => {
      if (!m.municao || !m.v0) return;
      const chave = m.municao.toLowerCase();
      (variam[chave] ??= new Set<number>()).add(m.v0);
    });
    const nVariam = Object.values(variam).filter((s) => s && s.size > 1).length;
    return h('div', { className: 'card a3tut-card a3arm-proc' },
      h('div', { className: 'a3tut-card__head' },
  ...
  function tabelaCarregadores(lista: readonly A3Carregador[]): HTMLDivElement {
    const colunas: readonly Coluna<A3Carregador>[] = [
  ...
  function tabelaTipo(tp: A3ArmaTipo, armas: readonly A3Arma[]): HTMLDivElement {
    const colunas: readonly Coluna<A3Arma>[] = [
  ...
  function procedenciaArmas(): HTMLDivElement {
    const m = exigirBase('A3ARM_META');
    const carregarArsenal = exigirBase('carregarArsenal');
    const pct = (n: number, d: number): string => `${((n / d) * 100).toFixed(0)}%`;
    const linha = (rot: string, valor: string, nota: string): HTMLDivElement =>
      h('div', { className: 'a3arm-proc__linha' },
  ...
  function secaoEl(
    cat: ChipCat & { readonly desc?: string }, cards: readonly HChild[], qtd: number, desc: string,
  ): HTMLDivElement {
  ...
  function render(): void {
    ...
    if (aba === 'mods') {
      total = A3TUT_TOTAL;
      const A3TUT_DUAL_ARMS = exigirBase('A3TUT_DUAL_ARMS');
      const A3TUT_CATEGORIAS = exigirBase('A3TUT_CATEGORIAS');
      const A3TUT_MODS = exigirBase('A3TUT_MODS');
      ...
      const modsEntradas = Object.entries(A3TUT_MODS)
        .flatMap(([id, t]) => (t ? [[id, t] as const] : []));
      A3TUT_CATEGORIAS.forEach((cat) => {
        if (catAtiva !== 'all' && catAtiva !== cat.id) return;
        const doCat = modsEntradas.filter(([, t]) => t.cat === cat.id);
      ...
      total = A3COL_TOTAL;
      const A3COL_INFO = exigirBase('A3COL_INFO');
      const A3COL_CATS = exigirBase('A3COL_CATS');
      const A3COL_ITENS = exigirBase('A3COL_ITENS');
      ...
      const itensEntradas = Object.entries(A3COL_ITENS)
        .flatMap(([id, it]) => (it ? [[id, it] as const] : []));
      A3COL_CATS.forEach((cat) => {
        if (catAtiva !== 'all' && catAtiva !== cat.id) return;
        const doCat = itensEntradas.filter(([, it]) => it.cat === cat.id);
      ...
      const A3ARM = exigirBase('A3ARM');
      const A3ARM_TIPOS = exigirBase('A3ARM_TIPOS');
      const A3ARM_META = exigirBase('A3ARM_META');
      const base: readonly A3Arma[] = arsenalMods ? [...A3ARM, ...arsenalMods] : A3ARM;
      total = base.length;
      ...
      const A3VEI = exigirBase('A3VEI');
      const A3VEI_META = exigirBase('A3VEI_META');
      total = A3VEI.length;
      ...
      contador.textContent = `${visiveis} de ${total} veículos do núcleo `
        + `(${A3VEI_META.porCategoria ? Object.values(A3VEI_META.porCategoria).reduce((a, b) => (a ?? 0) + (b ?? 0), 0) : 0} no acervo com mods)`;
    } else if (aba === 'equipamento') {
      const A3EQP = exigirBase('A3EQP');
      total = A3EQP.length;
      ...
      import('./arma3-extracao-painel')
        ...
        .catch((e: unknown) => {
          ...
          corpo.replaceChildren(h('p', { className: 'u-text-danger' },
            `Falha ao abrir o painel: ${msg}`));
        });
