...
import type {
  WikiArtigo, WikiPortal, WikiFichaArma, WikiFichaAcessorio, WikiFichaTerreno,
  WikiFichaVeiculo, WikiFichaEquipamento, WikiFichaSoldado, WikiTipo
} from '../data/wiki-arma3.js';

/** Um estado linkável da wiki. Tudo opcional: ausente significa "não entra na URL". */
interface EstadoWiki {
  readonly p?: string;
  readonly a?: string;
  readonly q?: string;
  readonly n?: number;
...
/** Uma linha da infobox: devolve `null` quando o valor não veio, e a linha some. */
type LinhaInfobox = (rot: string, val: HChild) => HTMLDivElement | null;

/** Par rótulo → destino do breadcrumb. Destino `null` é a migalha atual. */
type Migalha = readonly [rotulo: string, destino: string | null];

const portalPorId: Readonly<Record<string, WikiPortal | undefined>> =
  Object.fromEntries(WIKI_PORTAIS.map((p) => [p.id, p]));

/* Monta a URL de um estado da wiki — tudo navega por link, nada por estado oculto. */
function url({ p = '', a = '', q = '', n = 0, v = '' }: EstadoWiki = {}): string {
...
const ir = (estado: EstadoWiki): void => router.navigate(url(estado));
...
function capa(art: WikiArtigo, classe: string): HTMLElement {
  if (art.img) {
    return h('img', {
...
const selo = (art: WikiArtigo): HTMLSpanElement =>
  h('span', { className: `wk-selo wk-selo--n${art.nivel}` },
    WIKI_NIVEIS[art.nivel - 1].icon + ' ' + WIKI_NIVEIS[art.nivel - 1].nome);

/* Card do índice em modo grade. */
function cardArtigo(art: WikiArtigo): HTMLAnchorElement {
  return h('a', {
    className: 'card wk-card', href: '#' + url({ a: art.id }),
    onclick: (e: Event) => { e.preventDefault(); ir({ a: art.id }); }
  },
 ...
 * ============================================================== */
function vistaCapa(page: HTMLDivElement): void {
  ...
  page.appendChild(h('div', { className: 'wk-trilhas' },
    ...WIKI_NIVEIS.map((nv) => h('a', {
      ...
      onclick: (e: Event) => { e.preventDefault(); ir({ p: 'tudo', n: nv.id }); }
    },
  ...
  page.appendChild(h('div', { className: 'wk-portais' },
    ...WIKI_PORTAIS.map((p) => h('a', {
      ...
      onclick: (e: Event) => { e.preventDefault(); ir({ p: p.id }); }
    },
  ...
  page.appendChild(h('div', { className: 'card wk-colecao' },
    ...
    h('div', { className: 'wk-colecao__acoes' },
      ...
      h('a', {
        ...
        onclick: (e: Event) => { e.preventDefault(); ir({ p: 'colecao' }); }
      }, 'Ver os itens'),
      h('a', { className: 'btn', href: '#/arma3-tutorial' }, 'Modo tutorial (abas)'))));
}

/**
 * Formata número medido, ou devolve `null` quando o config não informou.
 *
 * Ausente **some** da ficha em vez de virar "0": zero é uma medida, e publicá-la
 * no lugar de "não declarado" seria inventar dado.
 */
function num(x: number | null | undefined, suf = '', casas = 0): string | null {
  return typeof x === 'number' ? `${x.toFixed(casas).replace(/\.0+$/, '')}${suf}` : null;
}
...
function fichaSoldado(s: WikiFichaSoldado, linha: LinhaInfobox): (HTMLDivElement | null)[] {
  return [
    ...
    linha('Variantes', (s.variantes ?? 0) > 1 ? `${s.variantes} classes com a mesma carga` : null),
    linha('Origem', s.dlc),
...
function fichaEquipamento(q: WikiFichaEquipamento, linha: LinhaInfobox): (HTMLDivElement | null)[] {
  const p = q.protecao;
  return [
    ...
    linha('Passagem (passThrough)', p && p.passagem != null
      ...
      : null),
    linha('Capacidade', num(q.capacidade)),
    linha('Contêiner', q.containerClass),
    linha('Massa', num(q.massa, '', 1)),
    linha('Variantes', (q.variantes ?? 0) > 1 ? `${q.variantes} classes com os mesmos números` : null),
    linha('Origem', q.dlc),
...
function fichaVeiculo(v: WikiFichaVeiculo, linha: LinhaInfobox): (HTMLDivElement | null)[] {
  const b = v.blindagem;
  return [
    linha('Categoria', v.categoria),
    linha('Lado', v.lado),
    linha('Facção', v.faccao),
    linha('Blindagem do casco', num(v.armor)),
    linha('Blindagem estrutural', num(v.armorEstrutural)),
    ...
    linha('Partes com blindagem relativa', b && b.relativas
      ...
      : null),
    linha('Velocidade máx.', num(v.maxSpeed, ' km/h')),
    linha('Potência', num(v.potencia, ' hp')),
    linha('Ocupantes', num(v.lotacao)),
    linha('Carga', num(v.cargaMax)),
    linha('Combustível', num(v.combustivel, '', 2)),
    linha('Sistemas de arma', num(v.armas)),
    linha('Custo (IA)', num(v.custo)),
    linha('Origem', v.dlc),
 ...
 * ══════════════════════════════════════════════════════════════ */
function painelExtracao(): HTMLDivElement {
  const nf = new Intl.NumberFormat('pt-BR');
  const n = (x: number | null | undefined): string => (typeof x === 'number' ? nf.format(x) : '—');
...
function buscaGrande(): HTMLDivElement {
  const input = h('input', {
    ...
    onkeydown: (e: KeyboardEvent) => {
      if (e.key === 'Enter' && input.value.trim()) ir({ p: 'tudo', q: input.value.trim() });
    }
  });
...
/** As colunas ordenáveis da tabela — cada id é um campo real do artigo. */
type ColunaId = 'titulo' | 'catNome' | 'nivel' | 'autor';
interface Coluna { readonly id: ColunaId; readonly rotulo: string }
interface Ordem { readonly col: ColunaId; readonly asc: boolean }

/** O recorte que veio da URL. `n` chega como texto quando vem da query. */
...
function vistaIndice(page: HTMLDivElement, { p, q, n, v }: FiltroInicial): void {
  const portalAtivo = p === 'tudo' ? '' : p;
  const info = portalPorId[portalAtivo];
  let termo = q || '';
  let cat = '';
  let nivel = Number(n) || 0;
  let modo: 'grade' | 'tabela' = v === 'tabela' ? 'tabela' : 'grade';
  let ordem: Ordem = { col: 'titulo', asc: true };
  ...
  const campo = h('input', {
    ...
    oninput: debounce((e: Event) => {
      const alvoBusca = e.target;
      if (alvoBusca instanceof HTMLInputElement) { termo = alvoBusca.value; pintar(); }
    }, 120)
  });
  ...
  const chipsNivel = h('div', { className: 'wk-chips' },
    ...[{ id: 0, nome: 'Todos os níveis', icon: '⬡' }, ...WIKI_NIVEIS].map((nv) =>
      h('button', {
        ...
        onclick: () => {
          nivel = nv.id;
          chipsNivel.querySelectorAll<HTMLElement>('.wk-chip').forEach((b) =>
  ...
  const qtdEls = new Map<string, HTMLSpanElement>();
  ...
  const chipsCat = h('div', { className: 'wk-chips' },
    ...[{ id: '', nome: 'Tudo', icon: '⬡' }, ...cats].map((c) => {
      ...
      const btn = h('button', {
        ...
        onclick: () => {
          cat = c.id;
          chipsCat.querySelectorAll<HTMLElement>('.wk-chip').forEach((b) =>
  ...
  function atualizarChips(): void {
    ...
    qtdEls.forEach((el, id) => {
      ...
      if (chip) chip.hidden = qtd === 0 && id !== cat;
    });
  }
  ...
  function tabela(lista: readonly WikiArtigo[]): HTMLDivElement {
...
/**
 * A classe do bloco técnico que o artigo carrega, na ordem em que a página
 * sempre consultou. No máximo **uma** das fichas está presente por artigo, então
 * a primeira que responder é a resposta.
 */
function classeDoBlocoTecnico(art: WikiArtigo): string | undefined {
  return art.arma?.classe ?? art.acessorio?.classe ?? art.veiculo?.classe
    ?? art.equipamento?.classe ?? art.soldado?.classe ?? art.terreno?.classe;
}

function vistaArtigo(page: HTMLDivElement, art: WikiArtigo): void {
  ...
  const classeDoArtigo = classeDoBlocoTecnico(art);
  const ctx: A3ClsContexto = art.soldado ? 'soldado' : (art.veiculo ? 'veiculo' : 'item');
  ...
  let sqf: HTMLElement | null = null;
  if (art.sqf) {
    /* o comando entra numa const: dentro do callback o narrowing de `art.sqf`
     * não sobrevive, e é ele que vai pro clipboard */
    const comando = art.sqf;
    const btn = h('button', {
      ...
      onclick: () => {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
          navigator.clipboard.writeText(comando).then(ok).catch(() => {});
        } else ok();
      }
    }, '⧉ copiar');
    sqf = h('section', { className: 'wk-art__bloco' },
      h('h2', { className: 'wk-art__h2' }, '▸ Comando (cole no console)'),
      h('div', { className: 'wk-sqf' }, h('pre', { className: 'wk-sqf__code' }, comando), btn));
  }
  ...
  const relEl = vejaTambem.length
    ? h('section', { className: 'wk-art__bloco' },
      ...
      h('div', { className: 'wk-rel' }, ...vejaTambem.map((o) => h('a', {
        ...
        onclick: (e: Event) => { e.preventDefault(); ir({ a: o.id }); }
...
const ROTULO_TIPO: Readonly<Record<WikiTipo, string>> = {
...
function infobox(art: WikiArtigo): HTMLElement {
  const linha: LinhaInfobox = (rot, val) => val
...
function fichaArma(a: WikiFichaArma, linha: LinhaInfobox): (HTMLDivElement | null)[] {
  return [
    linha('Calibre', a.calibre),
    linha('Velocidade de saída', num(a.v0, ' m/s', 1)),
    ...
    linha('Dano', num(a.dano, '', 1)),
    linha('Cadência', num(a.rpm, ' tiros/min')),
    linha('Carregador', num(a.capacidade)),
    linha('Zeragem máx.', num(a.zeroing, ' m')),
    linha('Massa', num(a.massa)),
...
function fichaAcessorio(c: WikiFichaAcessorio, linha: LinhaInfobox): (HTMLDivElement | null)[] {
  const fov = c.fov;
  return [
    ...
    linha('Campo de visão', fov
      ? h('span', null, h('code', null, String(fov.init ?? fov.min)),
        (fov.min != null && fov.max != null && fov.min !== fov.max)
          ...
          : null)
      : null),
    linha('Modos ópticos', fov && (fov.modos ?? 0) > 1 ? String(fov.modos) : null),
    linha('Coef. do silenciador', num(c.coefSilenciador, '', 2)),
    linha('Massa', num(c.massa, '', 1)),
    linha('Origem', c.dlc),
...
function fichaTerreno(t: WikiFichaTerreno, linha: LinhaInfobox): (HTMLDivElement | null)[] {
  const g = t.grade;
  return [
 ...
 * ============================================================== */
function migalhas(itens: readonly Migalha[]): HTMLElement {
  ...
  itens.forEach(([rotulo, destino], i) => {
    ...
    el.appendChild(destino
      ? h('a', {
        href: '#' + destino,
        onclick: (e: Event) => { e.preventDefault(); router.navigate(destino); }
      }, rotulo)
 ...
 *  Entrada da rota
 * ============================================================== */
interface WikiArma3PageArgs { query?: Readonly<Record<string, string>> | null }

export function wikiArma3Page(args: WikiArma3PageArgs = {}): HTMLDivElement {
  const q = args.query || {};
  const page = h('div', { className: 'page-wiki' });

  /* o id sai da query numa const: `WIKI_POR_ID[q.a]` duas vezes não estreita,
   * e o artigo encontrado é o mesmo objeto usado logo abaixo */
  const idArtigo = q.a || '';
  const artigo = idArtigo ? WIKI_POR_ID[idArtigo] : undefined;

  if (artigo) {
    vistaArtigo(page, artigo);
  } else if (idArtigo) {
    ...
    page.appendChild(h('div', { className: 'card wk-vazio' },
      ...
      h('p', { className: 'u-text-muted' }, `Procurei por "${idArtigo}" e não achei. Veja se está aqui:`)));
    const chute = buscar(idArtigo.replace(/^w-/, '').replace(/-/g, ' ')).slice(0, 8);
