/**
 * /taticas-estrategias — princípios, táticas e estrategistas militares.
 */
...
const TATICAS: readonly Tactica[] = [
...
const ESTRATEGISTAS: readonly Estrategista[] = [
...
export function taticasEstrategiasPage(): HTMLDivElement {
  return h('div', { className: 'tat-page page-wrap' },
    buildImmersiveHero({
      ...
      hudLeft: '🗺 DOUTRINA',
      hudRight: 'SUN TZU → DRONE'
    }),
    h('section', { className: 'tat-section' },
      h('h2', { className: 'tat-section-title' }, '⚖ Princípios da Guerra'),
      h('div', { className: 'tat-princ-grid' },
        ...PRINCIPIOS.map((principio) => h('div', { className: 'tat-princ' },
          h('div', { className: 'tat-princ__nome' }, principio.nome),
          h('p', { className: 'tat-princ__desc' }, principio.desc),
          h('span', { className: 'tat-princ__origem' }, principio.origem))))),
    h('section', { className: 'tat-section' },
      h('h2', { className: 'tat-section-title' }, '⚔ Táticas Clássicas e Modernas'),
      h('div', { className: 'tat-grid' },
        ...TATICAS.map((tactica) => h('div', { className: `tat-card tat-card--${tactica.cor}` },
          h('div', { className: 'tat-card__head' },
            h('span', { className: 'tat-card__nome' }, tactica.nome),
            h('span', { className: 'tat-card__era' }, tactica.era)),
          h('p', { className: 'tat-card__desc' }, tactica.desc),
          h('div', { className: 'tat-card__ex' }, `▸ ${tactica.exemplo}`))))),
    h('section', { className: 'tat-section' },
      h('h2', { className: 'tat-section-title' }, '📖 Grandes Estrategistas'),
      h('div', { className: 'tat-estr-grid' },
        ...ESTRATEGISTAS.map((estrategista) => h('div', { className: 'tat-estr' },
          h('div', { className: 'tat-estr__head' },
            h('span', { className: 'tat-estr__nome' }, estrategista.nome),
            h('span', { className: 'tat-estr__epoca' }, estrategista.epoca)),
          h('div', { className: 'tat-estr__obra' }, estrategista.obra),
          h('p', { className: 'tat-estr__ideia' }, estrategista.ideia))))));
}
