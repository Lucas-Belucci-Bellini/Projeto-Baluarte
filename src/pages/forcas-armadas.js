/**
 * /forcas-armadas — Forças Armadas do Mundo
 * Tabela interativa: país, efetivos, reserva, orçamento, ramos
 */
...
const COLS = [
  { key: 'pais',      label: 'País',              fmt: r => `${r.bandeira} ${r.pais}` },
  { key: 'efetivos',  label: 'Efetivos Ativos',   fmt: r => r.efetivos.toLocaleString('pt-BR') },
  { key: 'reserva',   label: 'Reserva',            fmt: r => r.reserva > 0 ? r.reserva.toLocaleString('pt-BR') : '—' },
  { key: 'orcamento', label: 'Orçamento (bi USD)', fmt: r => `$${r.orcamento}bi` },
  { key: 'pib',       label: '% PIB',              fmt: r => `${r.pib}%` },
  { key: 'ramos',     label: 'Ramos',              fmt: r => r.ramos.join(', ') },
];

export function forcasArmadasPage() {
  let sortKey = 'efetivos';
  let sortDir = -1;
  let search = '';

  const searchEl = h('input', {
    type: 'search', placeholder: '🔍 Buscar país…',
    className: 'forcas-search',
    oninput: e => { search = e.target.value.toLowerCase(); renderTable(); }
  });

  const tableBody = h('tbody');
  const tableEl = h('table', { className: 'forcas-table' },
    h('thead', null,
      h('tr', null,
        ...COLS.map(c => h('th', {
          className: `forcas-th${c.key === sortKey ? ' is-sorted' : ''}`,
          onclick: () => {
            if (sortKey === c.key) sortDir *= -1; else { sortKey = c.key; sortDir = -1; }
            document.querySelectorAll('.forcas-th').forEach(el => el.classList.remove('is-sorted'));
            event.currentTarget.classList.add('is-sorted');
            renderTable();
          }
  ...
  function renderTable() {
    const filtered = FORCAS.filter(r =>
      r.pais.toLowerCase().includes(search)
    ).sort((a, b) => {
    ...
    for (const r of filtered) {
      const tr = h('tr', { className: 'forcas-row' },
        ...COLS.map((c, i) => {
          const td = h('td', { className: `forcas-td${i === 0 ? ' forcas-td--pais' : ''}` }, c.fmt(r));
          if (c.key === 'efetivos') {
            const bar = h('div', { className: 'forcas-bar' },
              h('div', { className: 'forcas-bar__fill', style: `width:${Math.min(100, r.efetivos / 20350)}%` })
            );
            td.appendChild(bar);
          }
          return td;
  ...
  const stats = h('div', { className: 'forcas-stats' },
    h('div', { className: 'forcas-stat' },
      h('span', { className: 'forcas-stat__val' }, FORCAS.reduce((s, r) => s + r.efetivos, 0).toLocaleString('pt-BR')),
      h('span', { className: 'forcas-stat__lbl' }, 'Efetivos totais')
    ),
    h('div', { className: 'forcas-stat' },
      h('span', { className: 'forcas-stat__val' }, `$${FORCAS.reduce((s, r) => s + r.orcamento, 0).toLocaleString('pt-BR')}bi`),
