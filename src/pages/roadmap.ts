 ...
 *
 * A implementação é deliberadamente estática: os tipos protegem a estrutura
 * editorial sem introduzir estado global, integrações ou novos efeitos.
 */

import '../styles/roadmap.css';
import { h } from '../utils/helpers.js';

type JarvisStatus = 'done' | 'next';
type SiteStatus = 'active' | 'stable' | 'growing' | 'planned';
...
const NIVEIS_JARVIS: readonly JarvisLevel[] = [
  {
    num: 1,
    label: 'Presença',
    status: 'done',
...
const ROADMAP_SITE: readonly RoadmapArea[] = [
  {
    area: 'IA & Jarvis',
    icon: '◉',
    status: 'active',
...
const STATUS_META: Record<JarvisStatus | 'planned', StatusMeta> = {
  done: { text: 'Concluído', cls: 'done' },
  next: { text: 'Em breve', cls: 'next' },
  planned: { text: 'Planejado', cls: 'planned' }
};

const SITE_STATUS_BADGE: Record<SiteStatus, StatusMeta> = {
...
function jarvisCard(level: JarvisLevel): HTMLDivElement {
  const meta = STATUS_META[level.status] ?? STATUS_META.planned;
  return h('div', { className: `jarvis-level jarvis-level--${level.status}` },
    h('div', { className: 'jarvis-level__num' }, `Nível ${level.num}`),
    h('div', { className: 'jarvis-level__title' }, level.label),
    h('span', { className: `jarvis-level__badge jarvis-level__badge--${meta.cls}` },
      level.status === 'done' ? '✓ ' : '◎ ',
      meta.text),
    h('ul', { className: 'jarvis-level__list' },
      ...level.items.map((item) => h('li', null, item))));
}

function siteCard(area: RoadmapArea): HTMLDivElement {
  const badge = SITE_STATUS_BADGE[area.status] ?? SITE_STATUS_BADGE.planned;
  const bodyClass = area.done.length === 0
    ? 'site-card__body site-card__body--full'
    : 'site-card__body';

  return h('div', {
    className: `site-card${area.isNew ? ' site-card--new' : ''}`
  },
    h('div', { className: 'site-card__head' },
      h('span', { className: 'site-card__icon' }, area.icon),
      h('span', { className: 'site-card__name' }, area.area),
      h('span', { className: `site-card__status site-card__status--${badge.cls}` }, badge.text)),
    h('div', { className: bodyClass },
      area.done.length > 0 && h('div', { className: 'site-card__col site-card__col--done' },
        ...
        h('ul', null, ...area.done.map((item) => h('li', null, item)))),
      area.next.length > 0 && h('div', { className: 'site-card__col site-card__col--next' },
        h('div', { className: 'site-card__col-label' }, 'A seguir'),
        h('ul', null, ...area.next.map((item) => h('li', null, item))))));
}

export function roadmapPage(): HTMLDivElement {
  return h('div', { className: 'roadmap-page' },
    ...
    h('section', { className: 'roadmap-section' },
      ...
      h('div', { className: 'roadmap-grid--jarvis' }, ...NIVEIS_JARVIS.map(jarvisCard))),
    h('section', { className: 'roadmap-section' },
      h('div', { className: 'roadmap-section__header' },
        h('div', { className: 'roadmap-section__icon' }, '⬡'),
        h('div', null,
      ...
      h('div', { className: 'roadmap-grid--site' }, ...ROADMAP_SITE.map(siteCard))),
    h('section', { className: 'roadmap-section' },
      h('div', { className: 'roadmap-section__header' },
        h('div', { className: 'roadmap-section__icon' }, '◈'),
        h('div', null,
