...
function renderMarkdown(report) {
  ...
  const migrationStep = report.migration.canonicalPagesRemaining.length
    ? 'Continuar a migração das páginas restantes sem quebrar os gates V1 verdes.'
    : 'Manter os wrappers JS por compatibilidade e avançar para dados, utilitários, Core e integrações por contratos próprios.';
  const tiposV2Step = report.github.runs.some((run) => /v2|core|validation/i.test(run.workflow))
    ? 'Separar causas raiz de efeitos cascata ao interpretar os gates V2 observados no SHA.'
    : 'Coletar os gates V2 correspondentes ao SHA e registrar unknown quando a execução externa não estiver disponível.';
