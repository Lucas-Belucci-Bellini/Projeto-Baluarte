...
/// Só `ReadFiles` possui operação implementada neste corte.
...
impl RuntimePolicy {
    pub fn new(root: impl Into<PathBuf>, capabilities: impl IntoIterator<Item = Capability>) -> Self {
        Self { root: root.into(), capabilities: capabilities.into_iter().collect() }
    }
    ...
    fn confined_file(&self, relative: impl AsRef<Path>) -> Result<PathBuf, RuntimeError> {
        ...
        if relative.is_absolute() || relative.components().any(|c| matches!(c, Component::RootDir | Component::Prefix(_))) {
...
mod tests {
    ...
    fn capability_names_are_stable_and_round_trip() {
        let all = [Capability::ReadFiles, Capability::WriteFiles, Capability::Network, Capability::Database, Capability::SystemInfo, Capability::UserData, Capability::Execution];
        for capability in all { assert_eq!(Capability::from_str(capability.as_str()).unwrap(), capability); }
