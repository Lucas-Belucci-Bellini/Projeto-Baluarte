/** High-level session over RuntimeRequestClient. */
...
/** @param {RuntimeSessionClientTransport} client @param {RuntimeSessionEnvelope} envelope @returns {RuntimeSessionClient} */
export function criarRuntimeSession(client, envelope) {
  ...
  async function abrir() {
    if (aberta) return /** @type {RuntimeSessionSuccessResponse} */ ({ status: 'authorized' });
    const resposta = await client.request({ op: 'authorize', envelope });
    if (!resposta || typeof resposta !== 'object' || Array.isArray(resposta)) throw new Error('Runtime não autorizado');
    const resultado = /** @type {RuntimeSessionResponse & Record<string, unknown>} */ (resposta);
    if (resultado.status !== 'authorized') throw new Error(resultado.message || 'Runtime não autorizado');
    aberta = true;
  ...
  async function lerArquivo(modulo, path) {
    ...
    if (!resposta || typeof resposta !== 'object' || Array.isArray(resposta)) throw new Error('Runtime não conseguiu ler o arquivo');
    const resultado = /** @type {RuntimeSessionResponse & Record<string, unknown>} */ (resposta);
    if (resultado.status !== 'file') throw new Error(resultado.message || 'Runtime não conseguiu ler o arquivo');
    return /** @type {RuntimeSessionSuccessResponse} */ ({ ...resultado, status: 'file' });
  }

  async function fechar() { aberta = false; }
