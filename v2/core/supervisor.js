/** Read-only supervisor facade over boot state and health. */

/** @typedef {'idle'|'starting'|'ready'|'degraded'|'stopping'|'stopped'|'failed'} SupervisorState */
/** @typedef {{subir: () => Promise<{falhas: ReadonlyArray<unknown>}>, descer: () => Promise<void>, diagnostico: () => unknown}} SupervisorBoot */
...
export function criarSupervisor(boot, saude, { agora = () => Date.now() } = {}) {
  ...
  /** @type {SupervisorState} */ let estado = 'idle';
  let inicio = /** @type {number|null} */ (null);
  let ultimaFalha = /** @type {string|null} */ (null);
  /** @param {SupervisorState} novo */
  const mudar = (novo) => { if (!ESTADOS.includes(novo)) throw new Error(`estado inválido: ${novo}`); estado = novo; saude.definirEstado(novo); };
  async function iniciar() {
    if (estado === 'starting' || estado === 'ready' || estado === 'degraded') return { estado, idempotente: true, diagnostico: boot.diagnostico() };
    if (estado === 'stopping') throw new Error('não é possível iniciar durante shutdown');
    inicio = agora(); ultimaFalha = null; mudar('starting');
    try {
      const resultado = await boot.subir();
      mudar(resultado.falhas.length > 0 ? 'degraded' : 'ready');
      return { estado, duracaoMs: agora() - inicio, resultado, diagnostico: boot.diagnostico() };
    } catch (erro) { ultimaFalha = erro instanceof Error ? erro.message : String(erro); mudar('failed'); throw erro; }
  }
  async function parar() {
    if (estado === 'idle' || estado === 'stopped') { mudar('stopped'); return { estado, idempotente: true }; }
    if (estado === 'stopping') return { estado, idempotente: true };
    mudar('stopping');
    try { await boot.descer(); mudar('stopped'); return { estado }; }
    catch (erro) { ultimaFalha = erro instanceof Error ? erro.message : String(erro); mudar('failed'); throw erro; }
  }
  function status() { const diagnostico = boot.diagnostico(); const health = saude.retrato(); return { estado, inicio, duracaoMs: inicio === null ? null : agora() - inicio, ultimaFalha, health, diagnostico }; }
