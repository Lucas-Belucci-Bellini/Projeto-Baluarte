...
/** @typedef {{subir: () => Promise<{falhas: ReadonlyArray<unknown>}>, descer: () => Promise<void>, diagnostico?: () => unknown}} RuntimeBootFacade */
...
export function criarOrquestrador(boot, saude) {
  ...
  return {
    ...
    diagnostico: () => ({ supervisor: supervisor.status(), boot: boot.diagnostico?.() ?? null, health: saude.retrato?.() ?? saude.verificar?.() ?? null }),
    supervisor
  };
}
